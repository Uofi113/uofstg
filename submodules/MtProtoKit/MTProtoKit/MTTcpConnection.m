...
{
#if TARGET_OS_IPHONE
    // Only the long-lived data/update connection should be a VoIP socket.
    // Download workers use media usage keys (4 and above); enabling VoIP on
    // those short-lived sockets made avatar/file activity disturb scrolling.
    BOOL isPrimaryDataConnection = _usageCalculationInfo != nil &&
        [_usageCalculationInfo incomingWWANKey] == 0 &&
        [_usageCalculationInfo outgoingWWANKey] == 1 &&
        [_usageCalculationInfo incomingOtherKey] == 2 &&
    ...
    if ([[UIDevice currentDevice].systemVersion intValue] <= 6 && isPrimaryDataConnection) {
        ...
        [_socket performBlock:^{
            ...
            // A suspended app cannot send userspace MTProto pings.  Keep the
            // primary VoIP TCP mapping alive in the kernel so Wi-Fi routers do
            // not silently forget it, and so a dead path is detected without
            // keeping the application process running.  Media sockets are not
            // marked as primary and therefore never receive these options.
...
#ifdef TCP_KEEPALIVE
                // Keep the router/NAT mapping warm while iOS owns the VoIP
                // stream in deep sleep.  Four minutes was long enough for
                // some access points to forget the idle mapping, leaving the
                // process alive but unable to receive an update that should
                // wake it.  This is still kernel-level traffic on the single
                // primary MTProto socket; it does not keep the app executing.
        ...
        NSLog(@"IOS6PUSH voipSocket enabled=%d host=%@ port=%d", backgroundingEnabled ? 1 : 0, host, (int)port);
    }
#endif
