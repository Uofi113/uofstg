...
// NSInputStream is allowed to return fewer bytes than requested.  This is
// observable on iOS 6 even for an in-memory MTProto response, while newer iOS
// versions usually return the whole block.  All TL primitives must therefore
// finish the requested read before advancing to padding or the next object.
static NSInteger TLReadExactly(NSInputStream *stream, uint8_t *buffer, NSUInteger length)
{
    NSUInteger offset = 0;
    while (offset < length)
    {
        NSInteger count = [stream read:buffer + offset maxLength:length - offset];
        if (count <= 0)
            break;
...
- (int32_t)readInt32
{
    ...
    if (TLReadExactly(self, (uint8_t *)&value, 4) != 4)
    {
...
- (int64_t)readInt64
{
    ...
    if (TLReadExactly(self, (uint8_t *)&value, 8) != 8)
    {
...
- (NSData *)readData:(int)length
{
    ...
    int readLen = (int)TLReadExactly(self, bytes, (NSUInteger)length);
    if (readLen != length)
    {
...
- (NSString *)readString
{
    ...
    {
        length = 0;
        TLReadExactly(self, ((uint8_t *)&length) + 1, 3);
        length >>= 8;
