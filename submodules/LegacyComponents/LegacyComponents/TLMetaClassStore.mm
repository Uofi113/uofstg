...
// Built once together with the TL scheme and never mutated afterwards. Runtime
// parsing must not read the Objective-C-valued libc++ unordered_map: old iOS
// libc++/ARC can invalidate that table while MTProto queues are looking in it.
static NSDictionary *CodexManualObjectParsersSnapshot = nil;

static int32_t CodexCurrentManualSignature = 0;
// Diagnostic pointers are touched by multiple parsing queues. Avoid racing ARC
// strong stores; assigned values are literals or NSStringFromClass() results.
...
- (id<TLObject>)TLdeserialize:(NSInputStream *)is signature:(int32_t)signature environment:(id<TLSerializationEnvironment>)environment context:(TLSerializationContext *)__unused context error:(__autoreleasing NSError **)error
{
    switch (signature)
    {
        ...
        case (int32_t)0xdaad85b0:
        {
            ...
            int32_t flags = [is readInt32];
            CodexReadInt64Vector(is);                 // channels:Vector<long>
            if (flags & (1 << 1))
                CodexSkipStringVector(is);            // countries_iso2
            if (flags & (1 << 3))
                [is readString];                      // prize_description
            [is readInt32];                           // quantity
            [is readInt32];                           // months
            [is readInt32];                           // until_date
        ...
        case (int32_t)0xc6991068:
        {
            ...
            int32_t flags = [is readInt32];
            [is readInt64];                           // channel_id
            if (flags & (1 << 3))
                [is readInt32];                       // additional_peers_count
            [is readInt32];                           // launch_msg_id
            [is readInt32];                           // winners_count
            [is readInt32];                           // unclaimed_count
            CodexReadInt64Vector(is);                 // winners:Vector<long>
            [is readInt32];                           // months
            if (flags & (1 << 1))
                [is readString];                      // prize_description
            [is readInt32];                           // until_date
...
void TLMetaClassStore::mergeScheme(TLScheme *scheme)
{
    ...
    {   
        ...
        NSMutableDictionary *manualParsersSnapshot = [[NSMutableDictionary alloc] initWithCapacity:(NSUInteger)manualObjectParsers.size()];
        for (std::unordered_map<int32_t, id<TLObject> >::const_iterator it = manualObjectParsers.begin(); it != manualObjectParsers.end(); ++it)
        {
            id<TLObject> parser = it->second;
            if (parser != nil)
                [manualParsersSnapshot setObject:parser forKey:[NSNumber numberWithInt:it->first]];
        }
        CodexManualObjectParsersSnapshot = [manualParsersSnapshot copy];
    }
}
...
TLConstructedValue TLMetaClassStore::constructValue(NSInputStream *is, int32_t signature, id<TLSerializationEnvironment> environment, TLSerializationContext *context, __autoreleasing NSError **error)
{
    ...
    id<TLObject> parser = [CodexManualObjectParsersSnapshot objectForKey:[NSNumber numberWithInt:signature]];
    if (parser != nil)
    {
