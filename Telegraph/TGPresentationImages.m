...
static UIImage *TGClassicIOS6Image(NSString *name)
{
    NSString *baseName = [name stringByDeletingPathExtension];
    NSString *extension = name.pathExtension.length == 0 ? @"png" : name.pathExtension;
    CGFloat scale = [UIScreen mainScreen].scale;
    NSString *resourceName = scale > 1.5f ? [baseName stringByAppendingString:@"@2x"] : baseName;
    NSString *path = [[NSBundle mainBundle] pathForResource:resourceName ofType:extension inDirectory:@"ClassicIOS6"];
    bool retinaFallback = false;
    if (path.length == 0 && scale <= 1.5f)
    {
        resourceName = [baseName stringByAppendingString:@"@2x"];
        path = [[NSBundle mainBundle] pathForResource:resourceName ofType:extension inDirectory:@"ClassicIOS6"];
        retinaFallback = path.length != 0;
    }
    UIImage *image = path.length == 0 ? nil : [UIImage imageWithContentsOfFile:path];
    if (image != nil && (scale > 1.5f || retinaFallback) && image.CGImage != NULL)
        image = [UIImage imageWithCGImage:image.CGImage scale:2.0f orientation:UIImageOrientationUp];
    return image;
}

static UIImage *TGClassicIOS6StretchableImage(NSString *name, NSInteger leftCap, NSInteger topCap)
{
    UIImage *image = TGClassicIOS6Image(name);
    return image == nil ? nil : [image stretchableImageWithLeftCapWidth:leftCap topCapHeight:topCap];
}
...
{
    if (true)
        return TGClassicIOS6StretchableImage(@"TabBarBadge.png", 10, 0);
...
{
    if (true)
        return TGClassicIOS6StretchableImage(@"DialogListUnreadBadge.png", 10, 0);
...
{
    if (true)
        return TGClassicIOS6StretchableImage(@"Msg_In.png", 20, 15);
...
{
    if (true)
        return TGClassicIOS6StretchableImage(@"Msg_In_Selected.png", 20, 15);
...
{
    if (true)
        return TGClassicIOS6StretchableImage(@"Msg_In_High.png", 23, 15);
...
{
    if (true)
        return TGClassicIOS6StretchableImage(@"Msg_In_High_Selected.png", 23, 15);
...
{
    if (true)
        return TGClassicIOS6StretchableImage(@"Msg_Out.png", 15, 15);
...
{
    if (true)
        return TGClassicIOS6StretchableImage(@"Msg_Out_Selected.png", 15, 15);
...
{
    if (true)
        return TGClassicIOS6StretchableImage(@"Msg_Out_High.png", 17, 15);
...
{
    if (true)
        return TGClassicIOS6StretchableImage(@"Msg_Out_High_Selected.png", 17, 15);
