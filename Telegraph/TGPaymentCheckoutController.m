...
#else
@interface TGPaymentCheckoutController () {
#endif
...
- (instancetype)initWithMessage:(TGMessage *)message {
    ...
    if (self != nil) {
        ...
        if (NSClassFromString(@"PKPaymentButton") != nil) {
            ...
            [(UIControl *)_applePayButton addTarget:self action:@selector(payPressed) forControlEvents:UIControlEventTouchUpInside];
        }
#endif
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
    ((UIControl *)_applePayButton).enabled = buttonEnabled;
#endif
        ...
        if ([paymentMethod isKindOfClass:[TGPaymentMethodApplePay class]]) {
            ...
            if (request.merchantIdentifier != nil) {
                ...
                for (TGInvoicePrice *price in _paymentForm.invoice.prices) {
                    ...
                    [items addObject:[(id)NSClassFromString(@"PKPaymentSummaryItem") summaryItemWithLabel:price.label amount:[[NSDecimalNumber alloc] initWithDouble:price.amount * 0.01]]];
                }
                if (_currentShippingOption != nil) {
                    for (TGInvoicePrice *price in _currentShippingOption.prices) {
                        totalAmount += price.amount;
                ...
                [items addObject:[(id)NSClassFromString(@"PKPaymentSummaryItem") summaryItemWithLabel:_bot.displayName amount:[[NSDecimalNumber alloc] initWithDouble:totalAmount * 0.01]]];
...
#endif
        if ([paymentMethod isKindOfClass:[TGPaymentMethodStripeToken class]]) {
...
- (void)continuePassKitPaymentWithCompletion:(void (^)(PKPaymentAuthorizationStatus status))completion payment:(PKPayment *)payment {
    if ([_paymentForm.nativeProvider isEqualToString:@"stripe"]) {
        ...
        configuration.appleMerchantIdentifier = @""; // Configure your own merchant ID for Apple Pay.
