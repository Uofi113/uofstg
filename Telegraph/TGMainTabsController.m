...
- (void)setPresentation:(TGPresentation *)presentation
{
    if ([TGPresentation classicIOS6Style])
    {
        ...
        _label.font = TGBoldSystemFontOfSize(11.0f);
...
- (void)setCount:(int)count
{
    ...
    else
    {
        ...
        if ([TGPresentation classicIOS6Style])
        {
            CGFloat retinaPixel = TGIsRetina() ? 0.5f : 0.0f;
            frame.size.width = MAX(20.0f, textWidth + 12.0f + retinaPixel * 2.0f);
            ...
            labelFrame.origin.x = frame.origin.x + 6.0f + retinaPixel;
            labelFrame.origin.y = 4.0f + retinaPixel;
...
@interface TGTabBarButton : UIView
{
    ...
    UIImage *_classicSelectedImage;
    CAGradientLayer *_classicSelectionLayer;
}
...
- (instancetype)initWithImage:(UIImage *)image title:(NSString *)title presentation:(TGPresentation *)presentation
{
    ...
    if (self != nil)
    {
        ...
        _label = [[UILabel alloc] init];
        _label.backgroundColor = [UIColor clearColor];
        _label.textColor = [TGPresentation classicIOS6Style] ? UIColorRGB(0x999999) : presentation.pallete.tabTextColor;
...
- (void)setImage:(UIImage *)image presentation:(TGPresentation *)presentation
{
    ...
    _label.textColor = UIColorRGB(0x999999);
    _label.highlightedTextColor = UIColorRGB(0xffffff);
    _label.shadowColor = [UIColor clearColor];
    _label.shadowOffset = CGSizeZero;
    [self setNeedsLayout];
}
...
- (void)layoutSubviews
{
    ...
    if (_landscape)
    {
        ...
        CGFloat width = CGCeil(_imageView.frame.size.width + 6.0f + _label.frame.size.width);
    ...
    {
        _label.font = TGBoldSystemFontOfSize(TGIsPad() ? 11.0f : 10.0f);
        if (!CGAffineTransformIsIdentity(_imageView.transform))
        {
            [UIView animateWithDuration:0.2 animations:^
            {
...
- (instancetype)initWithFrame:(CGRect)frame presentation:(TGPresentation *)presentation
{
    ...
    if (self != nil)
    {
        ...
        [_contactsButton setClassicSelectedImage:TGMainTabsClassicIOS6Image(@"TabIconContacts_Highlighted")];
        [_chatsButton setClassicSelectedImage:TGMainTabsClassicIOS6Image(@"TabIconMessages_Highlighted")];
        [_settingsButton setClassicSelectedImage:TGMainTabsClassicIOS6Image(@"TabIconSettings_Highlighted")];
...
{    
    ...
    CGFloat buttonWidth = CGFloor(width / (CGFloat)buttonsCount);
...
- (void)layoutSubviews
{
    ...
    _stripeView.frame = CGRectMake(0, 0.0f, viewSize.width, stripeHeight);
