...
#import "../legacy/TL/TLInputPeer.h"
...
static inline bool TGIOS6ConversationPeerIdIsModernRawChannel(int64_t peerId)
{
...
{
    return TGPeerIdIsChannel(peerId) || TGIOS6ConversationPeerIdIsModernRawChannel(peerId);
}

static inline int64_t TGIOS6ConversationChannelIdFromPeerId(int64_t peerId)
{
    if (TGIOS6ConversationPeerIdIsModernRawChannel(peerId))
...
#import "../submodules/LegacyComponents/LegacyComponents/TGWallpaperInfo.h"
...
#import "../submodules/LegacyComponents/LegacyComponents/TGStickerAssociation.h"

#import <map>
#import <vector>

#import "../thirdparty/WebP.framework/Headers/decode.h"
...
#import "../submodules/LegacyComponents/LegacyComponents/TGMediaAsset.h"
#import "../submodules/LegacyComponents/LegacyComponents/TGVideoEditAdjustments.h"
...
- (instancetype)initWithConversation:(TGConversation *)conversation mayHaveUnreadMessages:(bool)mayHaveUnreadMessages
{
    ...
    if (self != nil)
    {
        ...
        IOS6Trace(@"IOS6FULL chat.init peer=%lld title=%@ access=%lld unread=%d serviceUnread=%d kind=%d channel=%d group=%d mayUnread=%d", conversation.conversationId, conversation.chatTitle, conversation.accessHash, conversation.unreadCount, conversation.serviceUnreadCount, (int)conversation.kind, conversation.isChannel ? 1 : 0, conversation.isChannelGroup ? 1 : 0, mayHaveUnreadMessages ? 1 : 0);
...
/*#ifdef DEBUG
    ...
    _botReplyMarkupDisposable = [[[TGDatabaseInstance() signalBotReplyMarkupForPeerId:_conversationId] deliverOn:[SQueue mainQueue]] startWithNext:^(TGBotReplyMarkup *markup)
    {
        ...
        if (strongSelf != nil)
        {
            TGLog(@"IOS6BOT companion markup peer=%lld markup=%@ rows=%d inline=%d msg=%d user=%d", (long long)strongSelf->_conversationId, markup, (int)markup.rows.count, markup.isInline ? 1 : 0, markup.messageId, markup.userId);
            TGModernConversationController *controller = strongSelf.controller;
...
- (void)_controllerDidAppear:(bool)firstTime
{
    ...
    {   
        ...
        TGModernConversationController *controller = self.controller;
        while (false) TGLog(@"IOS6READ appear peer=%lld first=%d preview=%d canRead=%d", _conversationId, firstTime ? 1 : 0, self.previewMode ? 1 : 0, [controller canReadHistory] ? 1 : 0);
        if (!self.previewMode && [controller canReadHistory])
        {
            ...
            //if (![self supportsSequentialRead]) {
                while (false) TGLog(@"IOS6READ appear.enqueue peer=%lld", _conversationId);
        ...
        else if (!self.previewMode)
        {
            [TGModernConversationCompanion dispatchOnMessageQueue:^
            {
                while (false) TGLog(@"IOS6READ appear.defer peer=%lld", _conversationId);
...
- (void)_updateMessageItemsWithData:(NSArray *)items
{
    ...
    for (id item in items)
    {
        ...
        if ([item isKindOfClass:TGMessageModernConversationItemClass])
        {
            ...
            if (needsAuthor && messageItem->_author == nil)
            {
                ...
                if (peerId != 0)
                {
                    if (TGIOS6ConversationPeerIdIsChannelLike(peerId)) {
    ...
    for (int itemIndex : requiredUsersItemIndices)
    {
        ...
        if (needsAuthor && messageItem->_message.fromUid != 0)
        {
            ...
            {
                int32_t authorUid = (int32_t)messageItem->_message.fromUid;
                it = pUsers->find(authorUid);
                if (it != pUsers->end())
                {
                ...
                {
                    TGUser *fallbackUser = [TGDatabaseInstance() loadUser:authorUid];
                    if (fallbackUser != nil)
                    {
                        messageItem->_author = fallbackUser;
                        if (_conversationId == -6284210382LL)
                            TGLog(@"IOS6AUTHOR fallback peer=%lld mid=%d from=%d name=%@ photo=%@", _conversationId, messageItem->_message.mid, authorUid, fallbackUser.displayName, fallbackUser.photoUrlSmall);
                    }
                    else if (_conversationId == -6284210382LL)
                    {
                        TGLog(@"IOS6AUTHOR missing peer=%lld mid=%d from=%d", _conversationId, messageItem->_message.mid, authorUid);
                    }
                }
            }
            else if (TGPeerIdIsChannel(messageItem->_message.fromUid) && channels[@(messageItem->_message.fromUid)])
            {
...
static TGMessageEntity *TGIOS6MessageEntityWithRange(TGMessageEntity *entity, NSRange range)
{
    if ([entity isKindOfClass:[TGMessageEntityBold class]])
        return [[TGMessageEntityBold alloc] initWithRange:range];
    if ([entity isKindOfClass:[TGMessageEntityBotCommand class]])
        return [[TGMessageEntityBotCommand alloc] initWithRange:range];
    if ([entity isKindOfClass:[TGMessageEntityCashtag class]])
        return [[TGMessageEntityCashtag alloc] initWithRange:range];
    if ([entity isKindOfClass:[TGMessageEntityCode class]])
        return [[TGMessageEntityCode alloc] initWithRange:range];
    if ([entity isKindOfClass:[TGMessageEntityEmail class]])
        return [[TGMessageEntityEmail alloc] initWithRange:range];
    if ([entity isKindOfClass:[TGMessageEntityHashtag class]])
        return [[TGMessageEntityHashtag alloc] initWithRange:range];
    if ([entity isKindOfClass:[TGMessageEntityItalic class]])
        return [[TGMessageEntityItalic alloc] initWithRange:range];
    if ([entity isKindOfClass:[TGMessageEntityMention class]])
        return [[TGMessageEntityMention alloc] initWithRange:range];
    if ([entity isKindOfClass:[TGMessageEntityMentionName class]])
        return [[TGMessageEntityMentionName alloc] initWithRange:range userId:((TGMessageEntityMentionName *)entity).userId];
    if ([entity isKindOfClass:[TGMessageEntityPhone class]])
        return [[TGMessageEntityPhone alloc] initWithRange:range];
    if ([entity isKindOfClass:[TGMessageEntityPre class]])
        return [[TGMessageEntityPre alloc] initWithRange:range language:((TGMessageEntityPre *)entity).language];
    if ([entity isKindOfClass:[TGMessageEntityTextUrl class]])
        return [[TGMessageEntityTextUrl alloc] initWithRange:range url:((TGMessageEntityTextUrl *)entity).url];
    if ([entity isKindOfClass:[TGMessageEntityUrl class]])
        return [[TGMessageEntityUrl alloc] initWithRange:range];
    
    return nil;
}

static NSArray *TGIOS6MessageEntitiesForPart(NSArray *entities, NSRange partRange)
{
    if (entities.count == 0)
        return nil;
    
    NSMutableArray *result = [[NSMutableArray alloc] init];
    NSUInteger partEnd = NSMaxRange(partRange);
    
    for (TGMessageEntity *entity in entities)
    {
        NSRange entityRange = entity.range;
        NSUInteger entityEnd = NSMaxRange(entityRange);
        NSUInteger intersectionStart = MAX(entityRange.location, partRange.location);
        NSUInteger intersectionEnd = MIN(entityEnd, partEnd);
        
        if (intersectionEnd <= intersectionStart)
            continue;
        
        NSRange shiftedRange = NSMakeRange(intersectionStart - partRange.location, intersectionEnd - intersectionStart);
        TGMessageEntity *partEntity = TGIOS6MessageEntityWithRange(entity, shiftedRange);
        if (partEntity != nil)
            [result addObject:partEntity];
    }
...
- (void)controllerWantsToSendTextMessage:(NSString *)text entities:(NSArray *)entities asReplyToMessageId:(int32_t)replyMessageId withAttachedMessages:(NSArray *)withAttachedMessages completeGroups:(NSSet *)completeGroups disableLinkPreviews:(bool)disableLinkPreviews botContextResult:(TGBotContextResultAttachment *)botContextResult botReplyMarkup:(TGBotReplyMarkup *)botReplyMarkup
{
    ...
    if (text.length != 0)
    {
        ...
        else
        {
            ...
            {
                NSRange partRange = NSMakeRange(i, MIN(messagePartLimit, text.length - i));
                NSString *substring = [text substringWithRange:partRange];
                if (substring.length != 0)
                {
                    NSString *resultingText = substring;
                    NSArray *parsedEntities = entities == nil ? [TGMessage entitiesForMarkedUpText:substring resultingText:&resultingText] : TGIOS6MessageEntitiesForPart(entities, partRange);
        ...
        if (withAttachedMessages.count != 0 && completeGroups.count > 0)
        {
            ...
            for (TGMessage *message in withAttachedMessages)
            {
                ...
                if (accessHashes[@(peerId)] != nil) {
                    accessHash = [accessHashes[@(peerId)] longLongValue];
                } else {
            ...
            for (NSDictionary *batch in batches)
            {
                ...
                int64_t fromPeerId = [batch[@"peerId"] longLongValue];
                int64_t fromPeerAccessHash = [batch[@"accessHash"] longLongValue];
...
- (NSDictionary *)imageDescriptionFromMediaAsset:(TGMediaAsset *)asset previewImage:(UIImage *)previewImage document:(bool)document fileName:(NSString *)fileName caption:(NSString *)caption entities:(NSArray *)entities allowRemoteCache:(bool)__unused allowRemoteCache
{
    ...
    NSString *assetIdentifier = asset.uniqueIdentifier;
    ...
    dict[@"thumbnailSize"] = [NSValue valueWithCGSize:dimensions];
...
- (NSDictionary *)videoDescriptionFromMediaAsset:(TGMediaAsset *)asset previewImage:(UIImage *)previewImage dimensions:(CGSize)dimensions duration:(NSTimeInterval)duration adjustments:(TGVideoEditAdjustments *)adjustments  document:(bool)document fileName:(NSString *)fileName stickers:(NSArray *)stickers caption:(NSString *)caption entities:(NSArray *)entities timer:(int32_t)timer
{
    ...
    dict[@"thumbnailSize"] = [NSValue valueWithCGSize:dimensions];
    dict[@"document"] = @((document || isAnimation));
    if (thumbnailData != nil)
        dict[@"thumbnailData"] = thumbnailData;
    ...
    [TGModernConversationCompanion dispatchOnMessageQueue:^
    {
        ...
        for (NSDictionary *imageDescription in imageDescriptions)
        {
            if (imageDescription[@"localImage"] != nil)
            {
                ...
                {
                    imageMessage.targetPeerId = [imageDescription[@"message"][@"cid"] longLongValue];
                    imageMessage.targetMessageId = [imageDescription[@"message"][@"mid"] intValue];
                    editMedia = true;
                }
            ...
            else if (imageDescription[@"assetImage"] != nil)
            {
                ...
                if (imageDescription[@"message"] != nil)
                {
                    assetImageMessage.targetPeerId = [imageDescription[@"message"][@"cid"] longLongValue];
                    assetImageMessage.targetMessageId = [imageDescription[@"message"][@"mid"] intValue];
                    editMedia = true;
                }
            ...
            else if (imageDescription[@"assetVideo"] != nil)
            {
                ...
                if (imageDescription[@"message"] != nil)
                {
                    assetVideoMessage.targetPeerId = [imageDescription[@"message"][@"cid"] longLongValue];
                    assetVideoMessage.targetMessageId = [imageDescription[@"message"][@"mid"] intValue];
                    editMedia = true;
                }
        ...
        for (NSDictionary *documentDescription in descriptions)
        {
            if (documentDescription[@"downloadDocument"] != nil)
            {
                ...
                if ([self controllerShouldCacheServerAssets] && documentAttachment != nil)
                {
                    ...
                    {
                        remoteDocumentMessage.targetPeerId = [documentDescription[@"message"][@"cid"] longLongValue];
                        remoteDocumentMessage.targetMessageId = [documentDescription[@"message"][@"mid"] intValue];
                    }
                    [preparedMessages addObject:remoteDocumentMessage];
                }
                else
                {
                    ...
                    if (documentDescription[@"message"] != nil)
                    {
                        downloadDocumentMessage.targetPeerId = [documentDescription[@"message"][@"cid"] longLongValue];
                        downloadDocumentMessage.targetMessageId = [documentDescription[@"message"][@"mid"] intValue];
                    }
                    [preparedMessages addObject:downloadDocumentMessage];
                }
            }
            else if (documentDescription[@"cloudDocument"] != nil)
            {
                ...
                else
                {
                    ...
                    if (documentDescription[@"message"] != nil)
                    {
                        cloudDocumentMessage.targetPeerId = [documentDescription[@"message"][@"cid"] longLongValue];
                        cloudDocumentMessage.targetMessageId = [documentDescription[@"message"][@"mid"] intValue];
                    }
...
- (void)controllerWantsToSendRemoteVideoWithMedia:(TGVideoMediaAttachment *)media asReplyToMessageId:(int32_t)replyMessageId text:(NSString *)text entities:(NSArray *)entities botContextResult:(TGBotContextResultAttachment *)botContextResult botReplyMarkup:(TGBotReplyMarkup *)botReplyMarkup
{
    if (media.videoId != 0)
    {
        ...
        if ([media.videoInfo urlWithQuality:0 actualQuality:NULL actualSize:&fileSize] != nil)
        {
            ...
            videoMessage.roundMessage = media.roundMessage;
...
- (void)controllerWantsToResendMessages:(NSArray *)messageIds
{
    [TGModernConversationCompanion dispatchOnMessageQueue:^
    {
        ...
        NSMutableDictionary *itemIndicesByMid = [[NSMutableDictionary alloc] initWithCapacity:_items.count];
        int itemIndex = -1;
        for (TGMessageModernConversationItem *messageItem in _items)
        {
            itemIndex++;
            [itemIndicesByMid setObject:@(itemIndex) forKey:@(messageItem->_message.mid)];
        }
        
        for (NSNumber *nMid in messageIds)
        {
            NSNumber *nIndex = [itemIndicesByMid objectForKey:nMid];
            if (nIndex != nil)
            {
                NSUInteger index = [nIndex unsignedIntegerValue];
                TGMessageModernConversationItem *currentItem = _items[index];
                [movingItems addObject:currentItem];
                [moveIndexFromIndex addObject:nIndex];
        ...
        for (int index = 0; index < (int)movingItems.count; index++)
        {
            ...
            NSNumber *nArrayIndex = [itemIndicesByMid objectForKey:@(messageItem->_message.mid)];
            NSUInteger arrayIndex = nArrayIndex == nil ? NSNotFound : [nArrayIndex unsignedIntegerValue];
...
- (void)controllerWantsToForwardMessages:(NSArray *)messageIndices
{
    [TGModernConversationCompanion dispatchOnMessageQueue:^
    {
        ...
        NSMutableSet *selectedMessageKeys = [[NSMutableSet alloc] initWithCapacity:messageIndices.count];
        for (TGMessageIndex *messageIndex in messageIndices)
        {
            [selectedMessageKeys addObject:[[NSString alloc] initWithFormat:@"%lld:%d", messageIndex.peerId, messageIndex.messageId]];
        }
        NSMutableSet *foundMessageKeys = [[NSMutableSet alloc] init];
        
        NSMutableDictionary *groups = [[NSMutableDictionary alloc] init];
        for (TGMessageModernConversationItem *messageItem in _items)
        {
            NSString *messageKey = [[NSString alloc] initWithFormat:@"%lld:%d", messageItem->_message.fromUid, messageItem->_message.mid];
            if ([selectedMessageKeys containsObject:messageKey])
            {
                [foundMessageKeys addObject:messageKey];
                
                if (TGIOS6ConversationPeerIdIsChannelLike(_conversationId) && messageItem->_message.cid != _conversationId) {
        ...
        for (TGMessageIndex *messageIndex in messageIndices)
        {
            NSString *messageKey = [[NSString alloc] initWithFormat:@"%lld:%d", messageIndex.peerId, messageIndex.messageId];
            if ([foundMessageKeys containsObject:messageKey])
                continue;
            
            int64_t peerId = (TGPeerIdIsAdminLog(_conversationId) && !TGIOS6ConversationPeerIdIsModernRawChannel(_conversationId)) ? messageIndex.peerId : _conversationId;
                ...
                case TGVideoMediaAttachmentType:
                {
                    ...
                    if (videoAttachment.videoId != 0)
                    {
                        ...
                        if ([videoAttachment.videoInfo urlWithQuality:0 actualQuality:NULL actualSize:&fileSize] != nil)
                        {
                            ...
                            remoteVideoMessage.roundMessage = videoAttachment.roundMessage;
                            if (!copyAssetsData)
    ...
    for (NSDictionary *desc in files)
    {
        ...
        else
        {
            ...
            if ([self isFileImage:fileName mimeType:desc[@"mimeType"] outAnimated:&isAnimatedImage])
            {
                ...
                else
                {
                    if (![desc[@"forceAsFile"] boolValue])
                    {
                        ...
                        if (desc[@"message"] != nil)
                        {
                            imageMessage.targetPeerId = [desc[@"message"][@"cid"] longLongValue];
                            imageMessage.targetMessageId = [desc[@"message"][@"mid"] intValue];
                        }
                        [preparedMessages addObject:imageMessage];
                    }
                }
            ...
            if (sendAsFile)
            {
                ...
                if (desc[@"message"] != nil)
                {
                    preparedMessage.targetPeerId = [desc[@"message"][@"cid"] longLongValue];
                    preparedMessage.targetMessageId = [desc[@"message"][@"mid"] intValue];
                }
                [preparedMessages addObject:preparedMessage];
            }
        }
...
- (void)_performFastScrollDown:(bool)becauseOfSendTextAction becauseOfNavigation:(bool)becauseOfNavigation
{
    [TGDatabaseInstance() dispatchOnDatabaseThread:^
    {
        [TGDatabaseInstance() loadMessagesFromConversation:_conversationId maxMid:INT_MAX maxDate:INT_MAX maxLocalMid:INT_MAX atMessageId:0 limit:20 extraUnread:false completion:^(NSArray *messages, __unused bool historyExistsBelow)
        {
...
- (void)controllerClearedConversation
{
    if (TGIOS6ConversationPeerIdIsChannelLike(_conversationId)) {
        ...
        int64_t rawChannelId = TGIOS6ConversationChannelIdFromPeerId(_conversationId);
        inputChannel.channel_id = rawChannelId < 0 ? (int64_t)(uint32_t)rawChannelId : rawChannelId;
        ...
        [[[[[TGTelegramNetworking instance] requestSignal:deleteHistory] mapToSignal:^SSignal *(id) {
            ...
            int64_t rawPeerChannelId = TGIOS6ConversationChannelIdFromPeerId(peerId);
            inputPeer.channel_id = rawPeerChannelId < 0 ? (int64_t)(uint32_t)rawPeerChannelId : rawPeerChannelId;
...
- (void)controllerCanReadHistoryUpdated
{
    ...
    while (false) TGLog(@"IOS6READ canReadUpdated peer=%lld preview=%d canRead=%d", _conversationId, self.previewMode ? 1 : 0, canReadHistory ? 1 : 0);
    if (canReadHistory)
    {
//        if ([self supportsSequentialRead]) {
//
//        } else {
            [TGModernConversationCompanion dispatchOnMessageQueue:^
            {
                if (_needsToReadHistory)
                {
                    ...
                    while (false) TGLog(@"IOS6READ deferred.enqueue peer=%lld", _conversationId);
                    [TGDatabaseInstance() transactionReadHistoryForPeerIds:@[[[TGReadPeerMessagesRequest alloc] initWithPeerId:_conversationId maxMessageIndex:nil date:0 length:0 unread:false]]];
                }
            }];
//        }
...
{
    IOS6Trace(@"IOS6FULL chat.loadMoreAbove.call peer=%lld manual=%d more=%d loading=%d items=%d", _conversationId, _manualMessageManagement ? 1 : 0, _moreMessagesAvailableAbove ? 1 : 0, _loadingMoreMessagesAbove ? 1 : 0, (int)_items.count);
    ...
    [TGModernConversationCompanion dispatchOnMessageQueue:^
    {
        if (_moreMessagesAvailableAbove && !_loadingMoreMessagesAbove)
        {
            ...
            IOS6Trace(@"IOS6FULL chat.loadMoreAbove.request peer=%lld minMid=%d minLocal=%d minDate=%d offset=%d path=/tg/conversations/(%lld)/history/(up%d)", _conversationId, minMid, minLocalMid, minDate, index, _conversationId, minMid);
        ...
        {
            IOS6Trace(@"IOS6FULL chat.loadMoreAbove.skip peer=%lld more=%d loading=%d", _conversationId, _moreMessagesAvailableAbove ? 1 : 0, _loadingMoreMessagesAbove ? 1 : 0);
        }
    }];
}
...
{
    IOS6Trace(@"IOS6FULL chat.loadMoreBelow.call peer=%lld manual=%d more=%d loading=%d items=%d", _conversationId, _manualMessageManagement ? 1 : 0, _moreMessagesAvailableBelow ? 1 : 0, _loadingMoreMessagesBelow ? 1 : 0, (int)_items.count);
    if (_manualMessageManagement)
        return;
    ...
    [TGModernConversationCompanion dispatchOnMessageQueue:^
    {
        if (_moreMessagesAvailableBelow && !_loadingMoreMessagesBelow)
        {
            ...
            IOS6Trace(@"IOS6FULL chat.loadMoreBelow.request peer=%lld maxMid=%d maxLocal=%d maxDate=%d path=/tg/conversations/(%lld)/history/(down%d)", _conversationId, maxMid, maxLocalMid, maxDate, _conversationId, maxMid);
        ...
        {
            IOS6Trace(@"IOS6FULL chat.loadMoreBelow.skip peer=%lld more=%d loading=%d", _conversationId, _moreMessagesAvailableBelow ? 1 : 0, _loadingMoreMessagesBelow ? 1 : 0);
        }
    }];
}
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    ...
    else if ([action isEqualToString:@"peerAvatarTapped"])
    {
        ...
        {
            [[TGInterfaceManager instance] navigateToConversationWithId:[options[@"peerId"] longLongValue] conversation:nil performActions:nil atMessage:nil clearStack:false openKeyboard:false canOpenKeyboardWhileInTransition:false navigationController:nil selectChat:false animated:true];
        }
        else
        {
            if (peerId != 0) {
        ...
        if (!alreadyProcessing)
        {
            [TGModernConversationCompanion dispatchOnMessageQueue:^
            {
                ...
                for (TGMessageModernConversationItem *messageItem in _items)
                {
                    ...
                    if (messageItem->_message.mid == mid && TGIOS6MessageMatchesMediaPeer(messageItem->_message, peerId))
                    {
                        foundMessage = true;
                        while (false) TGLog(@"IOS6MEDIA download request mid=%d peer=%lld cid=%lld from=%lld to=%lld available=%d attachments=%d", mid, peerId, messageItem->_message.cid, (int64_t)messageItem->_message.fromUid, (int64_t)messageItem->_message.toUid, messageItem->_mediaAvailabilityStatus ? 1 : 0, (int)messageItem->_message.mediaAttachments.count);
                ...
                if (!foundMessage)
                    while (false) TGLog(@"IOS6MEDIA download miss mid=%d peer=%lld items=%d", mid, peerId, (int)_items.count);
    ...
    else if ([action isEqualToString:@"transcribeAudioRequested"])
    {
        int32_t mid = [options[@"mid"] intValue];
        int64_t peerId = [options[@"peerId"] longLongValue];
            ...
            [TGModernConversationCompanion dispatchOnMessageQueue:^
            {
                ...
                else
                {
                    ...
                    for (TGMessageModernConversationItem *messageItem in _items)
                    {
                        ...
                        {
                            foundMessage = true;
                            while (false) TGLog(@"IOS6MEDIA cancel request mid=%d peer=%lld cid=%lld from=%lld to=%lld", mid, peerId, messageItem->_message.cid, (int64_t)messageItem->_message.fromUid, (int64_t)messageItem->_message.toUid);
                            TGDispatchOnMainThread(^
                            {
                                //TGModernConversationController *controller = self.controller;
                    ...
                    if (!foundMessage)
                        while (false) TGLog(@"IOS6MEDIA cancel miss mid=%d peer=%lld items=%d", mid, peerId, (int)_items.count);
    ...
    else if ([action isEqualToString:@"fastForwardMessage"])
    {
        ...
        {
            int64_t groupedId = [options[@"groupedId"] longLongValue];
        ...
        {
            [controller forwardMessages:@[ [TGMessageIndex indexWithPeerId:[options[@"peerId"] longLongValue] messageId:[options[@"mid"] intValue]] ] fastForward:true grouped:false];
        }
    }
...
{
    IOS6Trace(@"IOS6FULL chat.actorCompleted peer=%lld status=%d path=%@ result=%@", _conversationId, status, path, NSStringFromClass([result class]));
    NSString *sendMessagePathPrefix = [self _sendMessagePathPrefix];
    if ([path hasPrefix:[[NSString alloc] initWithFormat:@"/tg/conversations/(%@)/history/", [self _conversationIdPathComponent]]])
    {
        [TGModernConversationCompanion dispatchOnMessageQueue:^
        {
            if (status == ASStatusSuccess)
            {
                ...
                IOS6Trace(@"IOS6FULL chat.history.completed peer=%lld path=%@ messages=%d remote=%@ downwards=%@ clear=%@", _conversationId, path, (int)messages.count, result[@"remote"], result[@"downwards"], result[@"clearExisting"]);
                enum {
            ...
            {
                bool above = [path hasPrefix:[[NSString alloc] initWithFormat:@"/tg/conversations/(%@)/history/(up", [self _conversationIdPathComponent]]];
                bool below = [path hasPrefix:[[NSString alloc] initWithFormat:@"/tg/conversations/(%@)/history/(down", [self _conversationIdPathComponent]]];
                IOS6Trace(@"IOS6FULL chat.history.failed peer=%lld path=%@ above=%d below=%d", _conversationId, path, above ? 1 : 0, below ? 1 : 0);
                if (above)
                {
                    _loadingMoreMessagesAbove = false;
                    _moreMessagesAvailableAbove = false;
    ...
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        for (TGMediaAttachment *attachment in message.mediaAttachments)
        {
            if (attachment.type == TGVideoMediaAttachmentType)
            {
                ...
                if (url != nil)
                {
                    while (false) TGLog(@"IOS6MEDIA download request type=video peer=%lld mid=%d videoId=%lld high=%d url=%@", conversationId, message.mid, videoAttachment.videoId, highPriority ? 1 : 0, url);
                ...
                {
                    while (false) TGLog(@"IOS6MEDIA download missing-url type=video peer=%lld mid=%d videoId=%lld high=%d", conversationId, message.mid, videoAttachment.videoId, highPriority ? 1 : 0);
                }
            ...
            else if (attachment.type == TGDocumentMediaAttachmentType)
            {
                ...
                if (documentAttachment.documentId != 0 || documentAttachment.documentUri.length != 0)
                {
                    ...
                    while (false) TGLog(@"IOS6MEDIA download request type=document peer=%lld mid=%d docId=%lld dc=%d size=%d high=%d uri=%@", conversationId, message.mid, documentAttachment.documentId, documentAttachment.datacenterId, documentAttachment.size, highPriority ? 1 : 0, downloadUri);
...
//                            options[@"originInfo"] = [TGMediaOriginInfo mediaOriginInfoWithFileReference:nil fileReferences:nil url:((TGWebPageMediaAttachment *)attachment).url];
            ...
            else if (attachment.type == TGAudioMediaAttachmentType)
            {
                ...
                {
                    if (audioAttachment.ios6OriginInfo == nil && audioAttachment.audioId != 0)
                    {
                        audioAttachment.ios6OriginInfo = [TGMediaOriginInfo mediaOriginInfoWithFileReference:nil fileReferences:nil cid:conversationId mid:message.mid];
                        while (false) TGLog(@"IOS6MEDIA audio origin fallback peer=%lld mid=%d audioId=%lld", conversationId, message.mid, audioAttachment.audioId);
                    }
                    ...
                    while (false) TGLog(@"IOS6MEDIA download request type=audio peer=%lld mid=%d audioId=%lld dc=%d high=%d uri=%@", conversationId, message.mid, audioAttachment.audioId, audioAttachment.datacenterId, highPriority ? 1 : 0, audioAttachment.audioUri);
                ...
                {
                    while (false) TGLog(@"IOS6MEDIA download missing-id type=audio peer=%lld mid=%d high=%d", conversationId, message.mid, highPriority ? 1 : 0);
                }
            }
        }
    }];
                ...
                if (maybeReadContents && [self allowMessageForwarding])
                {
                    if (!item->_message.outgoing)
                    {
                        ...
                        if (!found)
                        {
                            ...
                            if (TGIOS6ConversationPeerIdIsChannelLike(_conversationId)) {
                                convType = 1;
                                convPeerId = (int32_t)TGIOS6ConversationChannelIdFromPeerId(_conversationId);
                            }
...
- (void)scheduleReadHistory {
    ...
    TGDispatchOnMainThread(^
    {
        ...
        {
            while (false) TGLog(@"IOS6READ schedule.enqueue peer=%lld", _conversationId);
            [TGModernConversationCompanion dispatchOnMessageQueue:^
            {
                [self _markIncomingMessagesAsReadSilent];
            }];
        ...
        else
        {
            [TGModernConversationCompanion dispatchOnMessageQueue:^
            {
                while (false) TGLog(@"IOS6READ schedule.defer peer=%lld", _conversationId);
                _needsToReadHistory = true;
            }];
        }
    });
...
+ (bool)canDeleteMessageForEveryone:(TGMessage *)message peerId:(int64_t)peerId isPeerAdmin:(bool)isPeerAdmin {
    ...
    else if (TGIOS6ConversationPeerIdIsChannelLike(peerId))
    {
