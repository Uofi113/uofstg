...
- (id)initWithUid:(int32_t)uid
{
    ...
    if (self)
    {
        ...
        TGButtonCollectionItem *uofistCreditItem = [[TGButtonCollectionItem alloc] initWithTitle:@"Modified by Uofist (@uofist)" action:NULL];
        TGButtonCollectionItem *lemonovosCreditItem = [[TGButtonCollectionItem alloc] initWithTitle:@"i.lemonovos.com" action:NULL];
        TGCollectionMenuSection *creditSection = [[TGCollectionMenuSection alloc] initWithItems:@[uofistCreditItem, lemonovosCreditItem]];
        [self.menuSections addSection:creditSection];
...
- (void)_updateProfileImage:(UIImage *)image
{
    ...
    TGImageProcessor filter = [TGRemoteImageView imageProcessorForName:@"avatar64"];
...
{
    TGLog(@"IOS6ARCHIVE account settings clear chat list cache");
