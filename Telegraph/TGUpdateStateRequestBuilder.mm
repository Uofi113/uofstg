...
static TLupdates_State *TGIOS6ValidDifferenceState(id state, NSString *source)
{
    ...
    TGLog(@"IOS6AUTH invalid difference state source=%@ class=%@ fallback pts=%d date=%d seq=%d qts=%d", source, stateClass, fallbackState.pts, fallbackState.date, fallbackState.seq, fallbackState.qts);
    IOS6Trace(@"IOS6FULL difference.invalidState source=%@ class=%@ fallbackPts=%d seq=%d", source, stateClass, fallbackState.pts, fallbackState.seq);
...
@property (nonatomic) int ios6UnchangedDifferenceSliceCount;
...
+ (void)applyUpdates:(NSArray *)addedMessagesDesc otherUpdates:(NSArray *)otherUpdates usersDesc:(NSArray *)usersDesc chatsDesc:(NSArray *)chatsDesc chatParticipantsDesc:(NSArray *)chatParticipantsDesc updatesWithDates:(NSArray *)updatesWithDates addedEncryptedActionsByPeerId:(NSDictionary *)addedEncryptedActionsByPeerId addedEncryptedUnparsedActionsByPeerId:(NSDictionary *)addedEncryptedUnparsedActionsByPeerId completion:(void (^)(bool))completion
{
    ...
    IOS6Trace(@"IOS6FULL updates.apply addedMessages=%d other=%d users=%d chats=%d participants=%d encrypted=%d encryptedRaw=%d withDates=%d",
        (int)addedMessagesDesc.count, (int)otherUpdates.count, (int)usersDesc.count, (int)chatsDesc.count, (int)chatParticipantsDesc.count, (int)addedEncryptedActionsByPeerId.count, (int)addedEncryptedUnparsedActionsByPeerId.count, (int)updatesWithDates.count);
    int traceOtherUpdateIndex = 0;
    for (id update in otherUpdates)
    {
        IOS6Trace(@"IOS6FULL updates.other[%d] class=%@", traceOtherUpdateIndex, NSStringFromClass([update class]));
        traceOtherUpdateIndex++;
    }
    int traceChatIndex = 0;
    for (TLChat *chatDesc in chatsDesc)
    {
        TGConversation *traceConversation = [[TGConversation alloc] initWithTelegraphChatDesc:chatDesc];
        IOS6Trace(@"IOS6FULL updates.chat[%d] tl=%@ raw=%d peer=%lld title=%@ channel=%d group=%d min=%d access=%lld kind=%d",
            traceChatIndex, NSStringFromClass([chatDesc class]), chatDesc.n_id, traceConversation.conversationId, traceConversation.chatTitle,
            traceConversation.isChannel ? 1 : 0, traceConversation.isChannelGroup ? 1 : 0, traceConversation.isMin ? 1 : 0, traceConversation.accessHash, (int)traceConversation.kind);
        traceChatIndex++;
    }
    ...
    NSMutableDictionary *updatePeerReadMarks = [[NSMutableDictionary alloc] init];
    ...
    for (TLUpdate *update in otherUpdates)
    {
        if ([update isKindOfClass:[TLUpdate$updateReadHistoryInbox class]])
        {
            ...
            TGLog(@"IOS6STATE updates.readInbox peer=%lld maxId=%d pts=%d ptsCount=%d", peerId, concreteUpdate.max_id, concreteUpdate.pts, concreteUpdate.pts_count);
        ...
        else if ([update isKindOfClass:[TLUpdate$updateReadHistoryOutbox class]])
        {
            ...
            TGLog(@"IOS6STATE updates.readOutbox peer=%lld maxId=%d pts=%d ptsCount=%d", peerId, concreteUpdate.max_id, concreteUpdate.pts, concreteUpdate.pts_count);
        ...
        else if ([update isKindOfClass:updateDeleteMessagesClass])
        {
            ...
            TGLog(@"IOS6STATE updates.deleteMessages count=%d pts=%d ptsCount=%d", (int)deleteMessages.messages.count, deleteMessages.pts, deleteMessages.pts_count);
        ...
        else if ([update isKindOfClass:updateMessageIdClass])
        {
            ...
            if (message.mid != 0) {
                ...
                TGLog(@"IOS6STATE updates.editMessage peer=%lld mid=%d pts=%d ptsCount=%d", message.cid, message.mid, updateEditMessage.pts, updateEditMessage.pts_count);
            }
        }
        else if ([update isKindOfClass:[TLUpdate$updateNewChannelMessage class]])
        {
            ...
            if ([toId isKindOfClass:[TLPeer$peerChannel class]]) {
                ...
                IOS6Trace(@"IOS6FULL updates.route newChannel peer=%lld raw=%d pts=%d ptsCount=%d msgClass=%@",
                    peerId, ((TLPeer$peerChannel *)toId).channel_id, channelMessage.pts, channelMessage.pts_count, NSStringFromClass([channelMessage.message class]));
            ...
            IOS6Trace(@"IOS6FULL updates.route channelTooLong peer=%lld raw=%d", peerId, channelTooLong.channel_id);
        ...
        else if ([update isKindOfClass:[TLUpdate$updateReadMessagesContents class]])
        {
            ...
            TGLog(@"IOS6CALL updatePhoneCall class=%@", NSStringFromClass([updatePhoneCall.phone_call class]));
            if ([updatePhoneCall.phone_call isKindOfClass:[TLPhoneCall$phoneCallRequested class]]) {
                ...
                TGLog(@"IOS6CALL update.requested call=%lld access=%lld admin=%d participant=%d date=%d", concreteCall.n_id, concreteCall.access_hash, concreteCall.admin_id, concreteCall.participant_id, concreteCall.date);
                TGLog(@"IOS6CALL update.requested.protocol class=%@ flags=%d min=%d max=%d libs=%@", NSStringFromClass([concreteCall.protocol class]), concreteCall.protocol.flags, concreteCall.protocol.min_layer, concreteCall.protocol.max_layer, concreteCall.protocol.library_versions);
                ...
                TGLog(@"IOS6CALL update.confirmed call=%lld access=%lld admin=%d participant=%d", concreteCall.n_id, concreteCall.access_hash, concreteCall.admin_id, concreteCall.participant_id);
                ...
                TGCallConnectionDescription *(^deserializeConnection)(id) = ^TGCallConnectionDescription *(id connection) {
                    if ([connection isKindOfClass:[TLPhoneConnection$phoneConnection class]]) {
                        ...
                        TGLog(@"IOS6CALL update.confirmed.connection id=%lld ip=%@ ipv6=%@ port=%d peerTagLen=%d", concreteConnection.n_id, concreteConnection.ip, concreteConnection.ipv6, concreteConnection.port, (int)concreteConnection.peer_tag.length);
                    ...
                    } else if ([connection isKindOfClass:[TLPhoneConnection$phoneConnectionWebrtc class]]) {
                        TLPhoneConnection$phoneConnectionWebrtc *webRtcConnection = (TLPhoneConnection$phoneConnectionWebrtc *)connection;
                        TGLog(@"IOS6CALL update.confirmed.webrtc id=%lld flags=%d turn=%d stun=%d ip=%@ ipv6=%@ port=%d user=%@ passLen=%d",
                ...
                for (id connection in concreteCall.connections) {
                    TGCallConnectionDescription *callConnection = deserializeConnection(connection);
                    if (callConnection != nil && defaultConnection == nil)
                        defaultConnection = callConnection;
                    else if (callConnection != nil)
                        [alternativeConnections addObject:callConnection];
                }
                TGLog(@"IOS6CALL update.confirmed.context legacyDefault=%@ legacyAlt=%d webrtc=%d customParametersLen=%d", defaultConnection, (int)alternativeConnections.count, (int)webrtcConnections.count, (int)customParameters.length);
                ...
                TGLog(@"IOS6CALL update.discarded call=%lld flags=%d reason=%@", concreteCall.n_id, concreteCall.flags, NSStringFromClass([concreteCall.reason class]));
                ...
                TGLog(@"IOS6CALL update.waiting call=%lld access=%lld admin=%d participant=%d", concreteCall.n_id, concreteCall.access_hash, concreteCall.admin_id, concreteCall.participant_id);
                ...
                TGLog(@"IOS6CALL update.accepted call=%lld access=%lld admin=%d participant=%d", concreteCall.n_id, concreteCall.access_hash, concreteCall.admin_id, concreteCall.participant_id);
        ...
        } else if ([update isKindOfClass:[TLUpdate$updatePhoneCallSignalingData class]]) {
            TLUpdate$updatePhoneCallSignalingData *signalingUpdate = (TLUpdate$updatePhoneCallSignalingData *)update;
            TGLog(@"IOS6CALL signaling.update call=%lld dataLen=%d", signalingUpdate.phone_call_id, (int)signalingUpdate.data.length);
            [TGTelegraphInstance.callManager handleSignalingDataForCallId:signalingUpdate.phone_call_id data:signalingUpdate.data];
    ...
    void (^continueBlock)() = ^
    {
        ...
        if (channelChats.count != 0) {
            __block NSMutableArray *channelsForRemoteRefresh = [[NSMutableArray alloc] init];
            [TGDatabaseInstance() dispatchOnDatabaseThread:^{
                NSMutableArray *knownChannels = [[NSMutableArray alloc] init];
                NSMutableArray *unknownChannels = [[NSMutableArray alloc] init];
                NSMutableArray *channelPeerIds = [[NSMutableArray alloc] initWithCapacity:channelChats.count];
                for (TGConversation *conversation in channelChats)
                    [channelPeerIds addObject:@(conversation.conversationId)];
                NSDictionary *existingChannels = [TGDatabaseInstance() loadChannels:channelPeerIds];
                ...
                for (TGConversation *conversation in channelChats) {
                    ...
                    TGConversation *existingChannel = existingChannels[@(conversation.conversationId)];
                    TGConversation *effectiveConversation = conversation;
                    
                    if (conversation.isMin && conversation.accessHash != 0) {
                        int64_t minAccessHash = conversation.accessHash;
                        [TGDatabaseInstance() setConversationCustomProperty:conversation.conversationId name:murMurHash32(@"ios6MinChannelAccessHash") value:[NSData dataWithBytes:&minAccessHash length:sizeof(minAccessHash)]];
                        IOS6Trace(@"IOS6FULL updates.channelAccessHashCandidate peer=%lld hash=%lld title=%@", conversation.conversationId, minAccessHash, conversation.chatTitle);
                        
                        if (existingChannel.accessHash != 0 && existingChannel.accessHash != minAccessHash) {
                            effectiveConversation = [conversation copy];
                            effectiveConversation.accessHash = existingChannel.accessHash;
                            IOS6Trace(@"IOS6FULL updates.channelAccessHashIgnoredMin peer=%lld min=%lld keep=%lld title=%@", effectiveConversation.conversationId, minAccessHash, effectiveConversation.accessHash, effectiveConversation.chatTitle);
                        } else {
                            IOS6Trace(@"IOS6FULL updates.channelAccessHashAcceptedMin peer=%lld hash=%lld existing=%lld title=%@", effectiveConversation.conversationId, minAccessHash, existingChannel.accessHash, effectiveConversation.chatTitle);
                        }
                    }
                    
                    if ((effectiveConversation.isMin || (!effectiveConversation.leftChat && !effectiveConversation.kickedFromChat)) && existingChannel == nil) {
                        [unknownChannels addObject:effectiveConversation];
                    } else {
                        [knownChannels addObject:effectiveConversation];
                    }
                    
                    if (effectiveConversation.feedId.intValue != 0)
                    {
                        TGFeed *feed = [TGDatabaseInstance() loadFeed:effectiveConversation.feedId.intValue];
                        if (feed == nil)
                ...
                NSMutableArray *unknownChannelsWithoutFeedUpdate = [[NSMutableArray alloc] init];
                NSMutableArray *unknownChannelsWithFeedUpdate = [[NSMutableArray alloc] init];
                for (TGConversation *conversation in unknownChannels) {
                    if ([peerIdsForUpdateChannels containsObject:@(conversation.conversationId)])
                        [unknownChannelsWithFeedUpdate addObject:conversation];
                    else
                        [unknownChannelsWithoutFeedUpdate addObject:conversation];
                    if (conversation.isMin && conversation.accessHash == 0) {
                        IOS6Trace(@"IOS6FULL updates.channelMinRemoteSkip peer=%lld title=%@", conversation.conversationId, conversation.chatTitle);
                        continue;
                    }
                    [channelsForRemoteRefresh addObject:conversation];
                }

                if (unknownChannelsWithoutFeedUpdate.count != 0)
                    [TGDatabaseInstance() updateChannels:unknownChannelsWithoutFeedUpdate updateFeeds:false];
                if (unknownChannelsWithFeedUpdate.count != 0)
                    [TGDatabaseInstance() updateChannels:unknownChannelsWithFeedUpdate updateFeeds:true];
            } synchronous:true];

            // Starting network refreshes while holding the database queue makes
            // chat opening wait behind unrelated startup work.
            for (TGConversation *conversation in channelsForRemoteRefresh) {
        ...
        [channelUpdatesByPeerId enumerateKeysAndObjectsUsingBlock:^(NSNumber *nPeerId, NSMutableArray *updates, __unused BOOL *stop) {
            ...
            TGLog(@"IOS6STATE updates.dispatchChannel peer=%lld updates=%d filteredMessageIds=%d", [nPeerId longLongValue], (int)updates.count, (int)filteredMessageIdUpdates.count);
        ...
        NSMutableDictionary *maxOutgoingReadDates = [[NSMutableDictionary alloc] init];
        ...
        NSMutableDictionary *removeMessageIdsByPeerId = [[NSMutableDictionary alloc] init];
        
        for (std::set<int>::iterator it = deleteMessageIds.begin(); it != deleteMessageIds.end(); it++)
        {
            int32_t messageId = *it;
            TGMessage *message = [TGDatabaseInstance() loadMessageWithMid:messageId peerId:0];
            NSNumber *nPeerId = message == nil ? @0 : @(message.cid);
        ...
        TGLog(@"IOS6STATE updates.transaction added=%d edited=%d messageUpdates=%d removePeers=%d chats=%d drafts=%d readIn=%d readOut=%d unreadMarks=%d webpages=%d",
            (int)addedMessages.count, (int)editedMessages.count, (int)messageUpdates.count, (int)removeMessageIdsByPeerId.count, (int)chatItems.count, (int)updatePeerDrafts.count, (int)maxIncomingReadIds.count, (int)maxOutgoingReadIds.count, (int)updatePeerReadMarks.count, (int)updatedWebpages.count);
...
{
    if ([difference isKindOfClass:[TLupdates_Difference$updates_differenceTooLong class]])
    {
        TGDatabaseState localState = [[TGDatabase instance] databaseState];
        int32_t serverPts = ((TLupdates_Difference$updates_differenceTooLong *)difference).pts;
        TGLog(@"IOS6STATE global.differenceTooLong localPts=%d serverPts=%d localSeq=%d -> full state resync", localState.pts, serverPts, localState.seq);
        self.cancelToken = [TGTelegraphInstance doRequestState:self];
        return;
    }
    ...
    dispatch_block_t continueBlock = ^
    {
        ...
        if ([difference isKindOfClass:[TLupdates_Difference$updates_differenceSlice class]])
        {
            ...
            if (!stateAdvanced)
            {
                self.ios6UnchangedDifferenceSliceCount++;
                if (self.ios6UnchangedDifferenceSliceCount >= 2)
                {
                    TGLog(@"IOS6STATE difference slice made no progress twice pts=%d date=%d qts=%d seq=%d -> state resync", safeState.pts, safeState.date, safeState.qts, safeState.seq);
                    self.cancelToken = [TGTelegraphInstance doRequestState:self];
                    return;
                }

                TGLog(@"IOS6STATE difference slice made no progress pts=%d date=%d qts=%d seq=%d -> one wider retry", safeState.pts, safeState.date, safeState.qts, safeState.seq);
            }
            else
            {
                self.ios6UnchangedDifferenceSliceCount = 0;
            }
            
            [[[TGApplyStateRequestBuilder alloc] initWithPath:[NSString stringWithFormat:@"/tg/service/applystate/(c%d)", applyStateCounter++]] execute:[[NSDictionary alloc] initWithObjectsAndKeys:[NSNumber numberWithInt:safeState.pts], @"pts", [NSNumber numberWithInt:safeState.date], @"date", [NSNumber numberWithInt:safeState.seq], @"seq", @(safeState.qts), @"qts", [NSNumber numberWithInt:-1], @"unreadCount", nil]];
            
            self.cancelToken = [TGTelegraphInstance doRequestStateDelta:safeState.pts date:safeState.date qts:safeState.qts requestBuilder:self];
        }
        else if ([difference isKindOfClass:[TLupdates_Difference$updates_difference class]] || [difference isKindOfClass:[TLupdates_Difference$updates_differenceEmpty class]])
        {
            ...
            if ([difference isKindOfClass:[TLupdates_Difference$updates_difference class]])
            {
                differenceSeq = safeState.seq;
                differenceDate = safeState.date;
                differenceQts = safeState.qts;
                TGLog(@"difference qts: %d", safeState.qts);
            }
            ...
            if ([difference isKindOfClass:[TLupdates_Difference$updates_difference class]])
            {
                ...
                NSInteger remoteUnreadCount = safeState.unread_count;
...
{
    IOS6NotificationProbe(@"DIFF", @"request_failed network=%d connecting=%d updating=%d",
...
- (void)stateRequestSuccess:(TLupdates_State *)state
{
    ...
    [ActionStageInstance() requestActor:[NSString stringWithFormat:@"/tg/dialoglist/(%d)", INT_MAX] options:[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:true], @"inline", [NSNumber numberWithInt:25], @"limit", nil] watcher:self];
}

- (void)stateRequestFailed
{
    // A failed getState has no valid _state. Continuing through dialog-list
    // completion used to apply an all-zero state and could leave the UI in an
    // endless Updating loop. Finish the failed synchronization normally and
    // let the networking layer retry when connectivity is available.
    [self stateDeltaRequestFailed];
}
