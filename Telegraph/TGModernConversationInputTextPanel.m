...
#import "../submodules/MtProtoKit/MTProtoKit/MTTime.h"
...
#import "../submodules/LegacyComponents/LegacyComponents/TGInputTextTag.h"

#import "../submodules/MtProtoKit/MTProtoKit/MTProtoKit.h"

#import "TGPresentation.h"

static UIImage *TGClassicIOS6InputPanelImage(NSString *name)
{
...
@interface TGModernConversationInputTextPanel () <HPGrowingTextViewDelegate, TGModernConversationInputAttachButtonDelegate, TGModernConversationInputMicButtonDelegate>
{
    ...
    CAGradientLayer *_classicIOS6BackgroundGradient;
...
- (void)setPresentation:(TGPresentation *)presentation
{
    ...
    UIImage *classicInputField = TGClassicIOS6InputPanelImage(@"InputField");
    if (classicInputField != nil)
    {
        // Preserve both rounded ends and stretch only the flat centre strip.
        if ([classicInputField respondsToSelector:@selector(resizableImageWithCapInsets:resizingMode:)])
            _fieldBackground.image = [classicInputField resizableImageWithCapInsets:UIEdgeInsetsMake(10.0f, 10.0f, 10.0f, 10.0f) resizingMode:UIImageResizingModeStretch];
        else
            _fieldBackground.image = [classicInputField stretchableImageWithLeftCapWidth:10 topCapHeight:10];
    }
    else
    ...
    [_micButton setImage:nil forState:UIControlStateHighlighted];
    _micButtonIconView.hidden = false;
    ...
    [_sendButton setImage:_messageEditingContext != nil ? presentation.images.chatInputConfirmIcon : presentation.images.chatInputSendIcon forState:UIControlStateNormal];
    [_sendButton setImage:nil forState:UIControlStateHighlighted];
    _sendButton.titleEdgeInsets = UIEdgeInsetsZero;
    _classicIOS6BackgroundGradient.hidden = true;
    ...
    UIKeyboardAppearance keyboardAppearance = presentation.pallete.isDark ? UIKeyboardAppearanceAlert : UIKeyboardAppearanceDefault;
    if (_inputField != nil && _inputField.internalTextView.keyboardAppearance != keyboardAppearance)
    {
        [UIView performWithoutAnimation:^
        {
            // Recreating the keyboard on iOS 6 loses the currently selected input language.
            bool shouldFlip = iosMajorVersion() >= 7 && _inputField.internalTextView.isFirstResponder;
            if (shouldFlip)
                [_inputField.internalTextView resignFirstResponder];
            _inputField.internalTextView.keyboardAppearance = keyboardAppearance;
            if (shouldFlip)
...
- (HPGrowingTextView *)inputField
{
    if (_inputField == nil)
    {
        ...
        _inputField.internalTextView.keyboardType = UIKeyboardTypeDefault;
...
{
    growingTextView.internalTextView.keyboardType = UIKeyboardTypeDefault;
...
- (void)growingTextViewDidChange:(HPGrowingTextView *)growingTextView afterSetText:(bool)afterSetText afterPastingText:(bool)afterPastingText
{
    ...
    // Programmatic text changes include clearing the field after sending. On old UIKit,
    // animating that transition can leave the Send layer visible behind the mic button.
    bool animated = !afterSetText;
...
- (void)updateKeyboardModeButtonExclusivity
{
    bool customKeyboardVisible = _customKeyboardView != nil;
    TGModernButton *buttonToHide = customKeyboardVisible ? _stickerModeButton : _keyboardModeButton;
    TGModernButton *buttonToShow = customKeyboardVisible ? _keyboardModeButton : _stickerModeButton;

    // These buttons occupy the same frame.  Keeping the outgoing button alive
    // during a keyboard transition makes both glyphs visible on older UIKit.
    removeViewAnimation(buttonToHide, @"transform");
    removeViewAnimation(buttonToHide, @"opacity");
    buttonToHide.alpha = 0.0f;
    buttonToHide.userInteractionEnabled = false;
...
{
    [self updateKeyboardModeButtonExclusivity];
...
- (void)updateSendButtonVisibility:(bool)animated
{
    ...
    if (animated)
    {
        if (appearingButton.layer.presentationLayer != nil)
            appearingButton.layer.transform = ((CALayer *)[appearingButton.layer presentationLayer]).transform;
        removeViewAnimation(appearingButton, @"transform");
        
        if (appearingButton.layer.presentationLayer != nil)
            appearingButton.layer.opacity = ((CALayer *)[appearingButton.layer presentationLayer]).opacity;
        removeViewAnimation(appearingButton, @"opacity");
        
        for (UIButton *button in disappearingButtons)
        {
            if (button.layer.presentationLayer != nil)
                button.layer.transform = ((CALayer *)[button.layer presentationLayer]).transform;
        ...
        {
            if (button.layer.presentationLayer != nil)
                button.layer.opacity = ((CALayer *)[button.layer presentationLayer]).opacity;
    ...
    else
    {
        ...
        [_sendButton.layer removeAllAnimations];
        ...
        [_micButton.layer removeAllAnimations];
        ...
        [_stickersArrowButton.layer removeAllAnimations];
...
- (void)updateMainLayout:(CGFloat)globalVerticalOffset
{
    ...
    if (_classicIOS6BackgroundGradient != nil)
        _classicIOS6BackgroundGradient.frame = _backgroundView.bounds;
    if (_classicIOS6BackgroundImageView != nil)
        _classicIOS6BackgroundImageView.frame = _backgroundView.bounds;
    ...
    setViewFrame(_sendButton, CGRectMake(frame.size.width - [self baseButtonWidth] - _safeAreaInset.right, globalVerticalOffset + frame.size.height - [self baseHeight], [self baseButtonWidth], [self baseHeight]));
...
- (void)setAssociatedPanel:(TGModernConversationAssociatedInputPanel *)associatedPanel animated:(bool)animated
{
    if (_associatedPanel != associatedPanel)
    {
        ...
        _associatedPanel.resultPreviewAppeared = ^{
            __strong TGModernConversationInputTextPanel *strongSelf = weakSelf;
            if (strongSelf != nil) {
                if (!TGIsPad() || [strongSelf->_associatedPanel isKindOfClass:[TGStickerAssociatedInputPanel class]])
...
- (void)keyboardModeButtonPressed
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self setCustomKeyboard:nil animated:false force:false];
        self.inputField.internalTextView.keyboardType = UIKeyboardTypeDefault;
        self.inputField.internalTextView.enableFirstResponder = true;
        [self.inputField becomeFirstResponder];
...
- (void)setReplyMarkup:(TGBotReplyMarkup *)replyMarkup
{
    if (!TGObjectCompare(_replyMarkup, replyMarkup))
    {
        ...
        bool shouldShowBotKeyboard = replyMarkup != nil && !replyMarkup.isInline && replyMarkup.rows.count != 0 && !replyMarkup.manuallyHidden && (!replyMarkup.hideKeyboardOnActivation || !replyMarkup.alreadyActivated);
        TGLog(@"IOS6BOT inputPanel setReplyMarkup rows=%d inline=%d enable=%d canAuto=%d shouldShow=%d custom=%@", (int)replyMarkup.rows.count, replyMarkup.isInline ? 1 : 0, _enableKeyboard ? 1 : 0, _canShowKeyboardAutomatically ? 1 : 0, shouldShowBotKeyboard ? 1 : 0, NSStringFromClass([_customKeyboardView class]));
        if (shouldShowBotKeyboard && ![_customKeyboardView isKindOfClass:[TGCommandKeyboardView class]])
        {
            _canShowKeyboardAutomatically = true;
...
- (void)customKeyboardInteraction:(CGFloat)offset
{
    ...
    CGFloat limit = fabsf(customKeyboardHeight - _contentAreaHeight + inputContainerHeight);
    CGFloat value = fabsf(offset);
