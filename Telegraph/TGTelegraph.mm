...
@interface TGConversationHistoryAsyncRequestActor (IOS6InvalidPeer)
...
static int64_t TGIOS6ArchivePeerIdAndType(TLPeer *peer, NSString **type, int32_t *rawId)
{
    if ([peer isKindOfClass:[TLPeer$peerUser class]])
    {
        int32_t userId = ((TLPeer$peerUser *)peer).user_id;
        if (type != NULL)
            *type = @"user";
        if (rawId != NULL)
            *rawId = userId;
    ...
    else if ([peer isKindOfClass:[TLPeer$peerChat class]])
    {
        int32_t chatId = ((TLPeer$peerChat *)peer).chat_id;
        if (type != NULL)
            *type = @"chat";
        if (rawId != NULL)
            *rawId = chatId;
        return TGPeerIdFromGroupId(chatId);
    }
    else if ([peer isKindOfClass:[TLPeer$peerChannel class]])
    {
        int32_t channelId = ((TLPeer$peerChannel *)peer).channel_id;
        if (type != NULL)
            *type = @"channel";
        if (rawId != NULL)
            *rawId = channelId;
        return TGPeerIdFromChannelId(channelId);
    }
    
    if (type != NULL)
        *type = NSStringFromClass([peer class]) ?: @"unknown";
    if (rawId != NULL)
        *rawId = 0;
    return 0;
}

static NSString *TGIOS6ArchiveDialogKey(TLDialog *baseDialog)
{
    ...
    NSString *type = nil;
    int32_t rawId = 0;
    TGIOS6ArchivePeerIdAndType(dialog.peer, &type, &rawId);
    if (type.length == 0 || rawId == 0)
        return nil;
    
    return [NSString stringWithFormat:@"%@:%d", type, rawId];
}

static void TGIOS6LogArchiveDialogs(TLmessages_Dialogs *dialogs, NSString *reason)
{
    TGLog(@"IOS6ARCHIVE %@ response=%@ dialogs=%d messages=%d chats=%d users=%d", reason ?: @"archive", NSStringFromClass([dialogs class]), (int)dialogs.dialogs.count, (int)dialogs.messages.count, (int)dialogs.chats.count, (int)dialogs.users.count);
    ...
    for (TLDialog *baseDialog in dialogs.dialogs)
    {
        if ([baseDialog isKindOfClass:[TLDialog$dialogMeta class]])
        {
            ...
            NSString *type = nil;
            int32_t rawId = 0;
            int64_t peerId = TGIOS6ArchivePeerIdAndType(dialog.peer, &type, &rawId);
            TGLog(@"IOS6ARCHIVE item[%d] type=%@ raw=%d peer=%lld top=%d unread=%d mentions=%d readIn=%d readOut=%d flags=%d pinned=%d serverFolder=%d", index, type ?: @"?", rawId, peerId, dialog.top_message, dialog.unread_count, dialog.unread_mentions_count, dialog.read_inbox_max_id, dialog.read_outbox_max_id, dialog.flags, (dialog.flags & (1 << 2)) != 0 ? 1 : 0, dialog.folder_id);
        }
        else
        {
            TGLog(@"IOS6ARCHIVE item[%d] unknown class=%@", index, NSStringFromClass([baseDialog class]));
        }
        index++;
    }
}
...
static NSArray *TGIOS6ArchivePeerIdsFromDialogs(TLmessages_Dialogs *dialogs)
{
    ...
    NSMutableSet *seenPeerIds = [[NSMutableSet alloc] init];
    ...
    for (TLDialog *baseDialog in dialogs.dialogs)
    {
        ...
        if (![seenPeerIds containsObject:nPeerId])
        {
            [seenPeerIds addObject:nPeerId];
...
static void TGIOS6StoreArchivePeerIds(TLmessages_Dialogs *dialogs, NSString *reason)
{
    ...
    TGLog(@"IOS6ARCHIVE peerIds.saved reason=%@ count=%d ids=%@", reason ?: @"archive", (int)peerIds.count, peerIds);
    [ActionStageInstance() dispatchResource:@"/dialogListReloaded" resource:@true];
}

@interface TGIOS6InputMessageID : NSObject <TLObject>

@property (nonatomic) int32_t messageId;

@end

@implementation TGIOS6InputMessageID

- (int32_t)TLconstructorSignature
{
    return (int32_t)0xa676a322;
}
...
extern "C" NSArray *TGIOS6ModernInputMessageIdsFromMessageIds(NSArray *messageIds)
{
    NSMutableArray *result = [[NSMutableArray alloc] initWithCapacity:messageIds.count];
    for (id item in messageIds)
    {
        if ([item respondsToSelector:@selector(TLconstructorSignature)])
        {
            [result addObject:item];
        }
        else
        {
            TGIOS6InputMessageID *inputMessage = [[TGIOS6InputMessageID alloc] init];
            inputMessage.messageId = [item intValue];
            [result addObject:inputMessage];
        }
    }
    return result;
}

@interface TGIOS6AuthSendCode : TLMetaRpc
...
@implementation TGIOS6AuthSendCode
...
- (void)TLserialize:(NSOutputStream *)os
{
    [os writeString:self.phoneNumber];
    [os writeInt32:self.apiId];
    [os writeString:self.apiHash];
    [os writeInt32:(int32_t)0xad253d78];
    [os writeInt32:0];
}

@end

@interface TGIOS6AuthSignIn : TLMetaRpc
...
@implementation TGIOS6AuthSignIn

- (int32_t)TLconstructorSignature
{
    return (int32_t)0x8d52a951;
}
...
- (void)TLserialize:(NSOutputStream *)os
{
    [os writeInt32:(1 << 0)];
    [os writeString:self.phoneNumber];
    [os writeString:self.phoneCodeHash];
    [os writeString:self.phoneCode];
}
...
static inline bool TGIOS6PeerIdIsModernRawChannel(int64_t peerId, int64_t accessHash)
{
    int64_t channelId = -peerId - 4294967296LL;
    return accessHash != 0 && channelId > 0 && channelId <= UINT32_MAX;
}

static int64_t TGIOS6StoredApiPeerId(int64_t peerId, NSString *propertyName)
{
    NSData *data = [TGDatabaseInstance() conversationCustomPropertySync:peerId name:murMurHash32(propertyName)];
...
static inline bool TGIOS6PeerIdIsChannel(int64_t peerId, int64_t accessHash)
{
    return TGIOS6PeerIdIsModernRawChannel(peerId, accessHash) || TGPeerIdIsChannel(peerId);
}

static inline int64_t TGIOS6FallbackChannelIdFromPeerId(int64_t peerId, int64_t accessHash)
{
    if (TGIOS6PeerIdIsModernRawChannel(peerId, accessHash))
...
{
    int64_t storedChannelId = TGIOS6StoredApiPeerId(peerId, @"ios6ApiChannelId");
    return storedChannelId > 0 ? storedChannelId : TGIOS6FallbackChannelIdFromPeerId(peerId, accessHash);
}

static inline int64_t TGIOS6ChatIdFromPeerId(int64_t peerId)
{
    int64_t storedChatId = TGIOS6StoredApiPeerId(peerId, @"ios6ApiChatId");
    if (storedChatId > 0)
        return storedChatId;

    return (int64_t)TGGroupIdFromPeerId(peerId);
}

static inline void TGIOS6FillInputChannel(TLInputChannel$inputChannel *inputChannel, int64_t peerId, int64_t accessHash)
{
    inputChannel.channel_id = TGIOS6ChannelIdFromPeerId(peerId, accessHash);
    inputChannel.access_hash = accessHash;
}

static int64_t TGIOS6ModernUserIdFromStoredUser(TGUser *user, int32_t uid)
{
    int64_t mappedUserId = TGModernUserIdForLegacyId(uid);
    if (mappedUserId != (int64_t)uid)
        return mappedUserId;
    ...
    if ([photoUrl hasPrefix:@"peerphoto:"])
    {
        ...
        if (components.count >= 4 && [components[2] intValue] == 1)
        {
            int64_t photoPeerId = [components[3] longLongValue];
            if (photoPeerId > 0)
                return photoPeerId;
        }
    }
...
@interface TGIOS6InputFolderPeer : NSObject <TLObject>
@property (nonatomic, strong) TLInputPeer *peer;
@property (nonatomic) int32_t folderId;
@end

@implementation TGIOS6InputFolderPeer
- (int32_t)TLconstructorSignature { return (int32_t)0xfbd2c296; }
- (int32_t)TLconstructorName { return -1; }
- (id<TLObject>)TLbuildFromMetaObject:(std::shared_ptr<TLMetaObject>)__unused metaObject { return self; }
- (void)TLfillFieldsWithValues:(std::map<int32_t, TLConstructedValue> *)__unused values { }
- (void)TLserialize:(NSOutputStream *)os
{
    TLMetaClassStore::serializeObject(os, self.peer, true);
    [os writeInt32:self.folderId];
}
@end

@interface TGIOS6EditPeerFoldersRequest : TLMetaRpc
@property (nonatomic, strong) NSArray *folderPeers;
@end

@implementation TGIOS6EditPeerFoldersRequest
- (int32_t)TLconstructorSignature { return (int32_t)0x6847d0ab; }
- (int32_t)TLconstructorName { return -1; }
- (Class)responseClass { return [TLUpdates class]; }
- (int)impliedResponseSignature { return 0; }
- (int)layerVersion { return 74; }
- (id<TLObject>)TLbuildFromMetaObject:(std::shared_ptr<TLMetaObject>)__unused metaObject { return self; }
- (void)TLfillFieldsWithValues:(std::map<int32_t, TLConstructedValue> *)__unused values { }
- (void)TLserialize:(NSOutputStream *)os
{
    [os writeInt32:(int32_t)0x1cb5c415];
    [os writeInt32:(int32_t)self.folderPeers.count];
    for (id<TLObject> peer in self.folderPeers)
        TLMetaClassStore::serializeObject(os, peer, true);
}
@end
...
- (id)init
{
    ...
    if (self != nil)
    {
        ...
        [ActionStageInstance() dispatchOnStageQueue:^
        {
            ...
            IOS6TraceSeparator([[NSString alloc] initWithFormat:@"launch %@ version %@", bundleIdentifier, [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleVersion"]]);
...
- (void)updateUserTypingStatuses
{
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        while (false) TGLog(@"===== Updating typing statuses");
...
- (void)processAuthorizedWithUserId:(int)uid clientIsActivated:(bool)clientIsActivated
{
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        ...
        if (_clientUserId != 0)
        {
            ...
            [_mediaBox setFetchResource:^SSignal *(id<MediaResource> resource, NSRange range) {
...
- (void)ios6PrepareAuthNetwork:(NSString *)reason
{
    TGLog(@"IOS6AUTH prepare network reason=%@ appState=%ld", reason, (long)[UIApplication sharedApplication].applicationState);
    [[TGTelegramNetworking instance] resume];
    [[TGTelegramNetworking instance] wakeUpWithCompletion:^
    {
        TGLog(@"IOS6AUTH wakeUp completed reason=%@", reason);
    }];
}

- (NSObject *)doSendConfirmationCode:(NSString *)phoneNumber requestBuilder:(TGSendCodeRequestBuilder *)requestBuilder
{
    [self ios6PrepareAuthNetwork:@"sendCode"];
    
    TGIOS6AuthSendCode *sendCode = [[TGIOS6AuthSendCode alloc] init];
    sendCode.phoneNumber = phoneNumber;
    sendCode.apiId = [_apiId intValue];
    sendCode.apiHash = _apiHash;
    
    NSString *langCode = [[NSLocale preferredLanguages] objectAtIndex:0];
    NSString *maskedPhone = phoneNumber.length <= 4 ? phoneNumber : [NSString stringWithFormat:@"%@***%@", [phoneNumber substringToIndex:MIN((NSUInteger)2, phoneNumber.length)], [phoneNumber substringFromIndex:phoneNumber.length - 2]];
    TGLog(@"IOS6AUTH sendCode request modern phone=%@ phoneLength=%lu apiId=%@ lang=%@", maskedPhone, (unsigned long)phoneNumber.length, _apiId, langCode);
    ...
    return [[TGTelegramNetworking instance] performRpc:sendCode completionBlock:^(id<TLObject> response, __unused int64_t responseTime, MTRpcError *error)
    {
        if (error == nil)
        {
            ...
            @try
            {
                codeType = [(id)response valueForKey:@"type"];
                nextType = [(id)response valueForKey:@"next_type"];
                timeout = [(id)response valueForKey:@"timeout"];
                phoneHash = [(id)response valueForKey:@"phone_code_hash"];
            }
            @catch (__unused NSException *exception)
            {
            }
            TGLog(@"IOS6AUTH sendCode success response=%@ type=%@ next=%@ timeout=%@ hashLen=%lu", response, codeType != nil ? NSStringFromClass([codeType class]) : nil, nextType != nil ? NSStringFromClass([nextType class]) : nil, timeout, (unsigned long)[phoneHash length]);
        ...
        else
        {
            ...
            TGLog(@"IOS6AUTH sendCode error raw=%@ type=%@ code=%d responseTime=%lld", error, errorType, (int)error.errorCode, responseTime);
...
{
    [self ios6PrepareAuthNetwork:@"resendSms"];
...
{
    [self ios6PrepareAuthNetwork:@"resendCall"];
...
{
    [self ios6PrepareAuthNetwork:@"signUp"];
...
{
    [self ios6PrepareAuthNetwork:@"signIn"];
    
    TGIOS6AuthSignIn *signIn = [[TGIOS6AuthSignIn alloc] init];
    signIn.phoneNumber = phoneNumber;
    signIn.phoneCodeHash = phoneHash;
...
#endif
        registerDevice.secret = [NSData data];
        registerDevice.other_uids = @[];
...
- (NSObject *)doRequestExtendedUserData:(int)uid actor:(TGExtendedUserDataRequestActor *)actor
{
    ...
    return [[TGTelegramNetworking instance] performRpc:getFullUser completionBlock:^(TLUserFull *result, __unused int64_t responseTime, MTRpcError *error)
    {
        ...
        {
            NSString *errorDescription = [error.errorDescription uppercaseString];
            if (error.errorCode == 400 && [errorDescription rangeOfString:@"USER_ID_INVALID"].location != NSNotFound)
            {
                TGUser *user = [TGDatabaseInstance() loadUser:uid];
                if (user != nil && user.phoneNumberHash != 0)
                {
                    TLRPCusers_getFullUser$users_getFullUser *retryGetFullUser = [[TLRPCusers_getFullUser$users_getFullUser alloc] init];
                    TLInputUser$inputUser *legacyInputUser = [[TLInputUser$inputUser alloc] init];
                    legacyInputUser.user_id = uid;
                    legacyInputUser.access_hash = user.phoneNumberHash;
                    retryGetFullUser.n_id = legacyInputUser;
                    TGLog(@"IOS6AUTH getFullUser retry legacy uid=%d hash=%lld after=%@", uid, user.phoneNumberHash, error.errorDescription);
                    
                    [[TGTelegramNetworking instance] performRpc:retryGetFullUser completionBlock:^(TLUserFull *retryResult, __unused int64_t retryResponseTime, MTRpcError *retryError)
                    {
                        if (retryError == nil)
                            [actor extendedUserDataRequestSuccess:retryResult];
                        else
...
{
    static bool hasPreviousDifferenceRequest = false;
    static int previousDifferencePts = 0;
    static int previousDifferenceDate = 0;
    static int previousDifferenceQts = 0;
    static NSTimeInterval previousDifferenceRequestTime = 0.0;
    int ptsLimit = 100;

    @synchronized([TGTelegraph class])
    {
        NSTimeInterval now = [[NSDate date] timeIntervalSince1970];
        if (hasPreviousDifferenceRequest && previousDifferencePts == pts && previousDifferenceDate == date && previousDifferenceQts == qts && now - previousDifferenceRequestTime < 30.0)
            ptsLimit = 500;

        hasPreviousDifferenceRequest = true;
        previousDifferencePts = pts;
        previousDifferenceDate = date;
        previousDifferenceQts = qts;
        previousDifferenceRequestTime = now;
    }
    ...
    // Normal requests stay small for single-core devices. If the server returns
    // a slice with exactly the same state, retry once with a wider window so a
    // channel-only prefix cannot trap the client in a tight request loop.
    getDifference.pts_limit = ptsLimit;
    getDifference.pts_total_limit = 1000;
    getDifference.date = date;
    getDifference.qts = qts;
    getDifference.qts_limit = 100;
    ...
    return [[TGTelegramNetworking instance] performRpc:getDifference completionBlock:^(id<TLObject> response, __unused int64_t responseTime, MTRpcError *error)
    {
        ...
        else if ([[[TGTelegramNetworking instance] extractNetworkErrorType:error] isEqualToString:@"PERSISTENT_TIMESTAMP_EMPTY"])
        {
            IOS6NotificationProbe(@"DIFF", @"rpc_error type=PERSISTENT_TIMESTAMP_EMPTY code=%d", error.errorCode);
            IOS6Trace(@"IOS6TRACE getDifference persistent timestamp empty pts=%d date=%d qts=%d -> refresh state", pts, date, qts);
            [self doRequestState:requestBuilder];
        }
        else
        {
            IOS6NotificationProbe(@"DIFF", @"rpc_error type=%@ code=%d description=%@ network=%d connecting=%d",
                                  [[TGTelegramNetworking instance] extractNetworkErrorType:error] ?: @"unknown",
                                  error.errorCode, error.errorDescription ?: @"none",
...
{
    return [self doRequestDialogsListWithOffset:offset limit:limit folderId:0 requestArchive:true requestBuilder:requestBuilder];
}

- (NSObject *)doSetConversationArchived:(int64_t)conversationId accessHash:(int64_t)accessHash archived:(bool)archived completion:(void (^)(bool success))completion
{
    TGIOS6InputFolderPeer *folderPeer = [[TGIOS6InputFolderPeer alloc] init];
    folderPeer.peer = [self createInputPeerForConversation:conversationId accessHash:accessHash];
    folderPeer.folderId = archived ? 1 : 0;

    TGIOS6EditPeerFoldersRequest *request = [[TGIOS6EditPeerFoldersRequest alloc] init];
    request.folderPeers = @[folderPeer];
    return [[TGTelegramNetworking instance] performRpc:request completionBlock:^(__unused TLUpdates *updates, __unused int64_t responseTime, MTRpcError *error)
    {
        if (completion != nil)
...
- (void)ios6RefreshArchivePeerIds
{
    TGLog(@"IOS6ARCHIVE manual refresh start");
    ...
    {
        TGIOS6LogArchiveDialogs(archiveDialogs, @"manual");
        TGIOS6StoreArchivePeerIds(archiveDialogs, @"manual");
        [[NSNotificationCenter defaultCenter] postNotificationName:@"TGIOS6ArchivePeerIdsUpdated" object:nil];
    } failure:^
    {
        TGLog(@"IOS6ARCHIVE manual refresh failed");
        [[NSNotificationCenter defaultCenter] postNotificationName:@"TGIOS6ArchivePeerIdsUpdated" object:nil];
    }];
}
...
- (NSObject *)doRequestDialogsListWithOffset:(TGDialogListRemoteOffset *)offset limit:(int)limit folderId:(int32_t)folderId requestArchive:(bool)requestArchive requestBuilder:(TGDialogListRequestBuilder *)requestBuilder
{
    IOS6Trace(@"IOS6TRACE dialogs request folder=%d offsetDate=%d offsetPeer=%lld offsetAccess=%lld offsetMid=%d limit=%d", folderId, offset.date, offset.peerId, offset.accessHash, offset.messageId, limit);
    IOS6Trace(@"IOS6FULL RPC dialogs.request folder=%d offsetDate=%d offsetPeer=%lld offsetAccess=%lld offsetMid=%d limit=%d", folderId, offset.date, offset.peerId, offset.accessHash, offset.messageId, limit);
    TLRPCmessages_getDialogs *getDialogs = [[TLRPCmessages_getDialogs alloc] init];
    getDialogs.flags |= (1 << 1);
    if (folderId == 1)
        getDialogs.flags |= (1 << 0);
    getDialogs.folder_id = folderId;
    ...
    getDialogs.hash = 0;
    IOS6Trace(@"IOS6FULL RPC dialogs.hash folder=%d hash=%lld", folderId, getDialogs.hash);
    if (folderId != 0)
        TGLog(@"IOS6ARCHIVE request archiveRpc=1 flags=0x%08x folder_id=%d limit=%d offsetDate=%d offsetPeer=%lld offsetMid=%d", getDialogs.flags, getDialogs.folder_id, getDialogs.limit, getDialogs.offset_date, offset.peerId, getDialogs.offset_id);
    ...
    return [[TGTelegramNetworking instance] performRpc:getDialogs completionBlock:^(TLmessages_Dialogs *dialogs, __unused int64_t responseTime, MTRpcError *error)
    {
        if (error == nil)
        {
            ...
            IOS6Trace(@"IOS6TRACE dialogs success folder=%d dialogs=%d messages=%d chats=%d users=%d response=%@", folderId, (int)dialogs.dialogs.count, (int)dialogs.messages.count, (int)dialogs.chats.count, (int)dialogs.users.count, NSStringFromClass([dialogs class]));
            IOS6Trace(@"IOS6FULL RPC dialogs.success folder=%d dialogs=%d messages=%d chats=%d users=%d response=%@", folderId, (int)dialogs.dialogs.count, (int)dialogs.messages.count, (int)dialogs.chats.count, (int)dialogs.users.count, NSStringFromClass([dialogs class]));
            int dialogIndex = 0;
            for (id dialog in dialogs.dialogs)
            {
                IOS6Trace(@"IOS6TRACE dialogs item[%d] class=%@", dialogIndex, NSStringFromClass([dialog class]));
                IOS6Trace(@"IOS6FULL RPC dialogs.item[%d] class=%@", dialogIndex, NSStringFromClass([dialog class]));
                dialogIndex++;
            }
            int chatIndex = 0;
            for (id chat in dialogs.chats)
            {
                IOS6Trace(@"IOS6TRACE dialogs chat[%d] class=%@", chatIndex, NSStringFromClass([chat class]));
                IOS6Trace(@"IOS6FULL RPC dialogs.chat[%d] class=%@", chatIndex, NSStringFromClass([chat class]));
                chatIndex++;
            }
            int userIndex = 0;
            for (id user in dialogs.users)
            {
                IOS6Trace(@"IOS6TRACE dialogs user[%d] class=%@", userIndex, NSStringFromClass([user class]));
                IOS6Trace(@"IOS6FULL RPC dialogs.user[%d] class=%@", userIndex, NSStringFromClass([user class]));
                userIndex++;
            }
        ...
        {
            IOS6Trace(@"IOS6TRACE dialogs failed folder=%d code=%d text=%@", folderId, error.errorCode, error.errorDescription);
            IOS6Trace(@"IOS6FULL RPC dialogs.failed folder=%d code=%d text=%@", folderId, error.errorCode, error.errorDescription);
...
- (NSObject *)doRequestDialogsListWithOffset:(TGDialogListRemoteOffset *)offset limit:(int)limit folderId:(int32_t)folderId requestArchive:(bool)requestArchive requestBuilder:(TGDialogListRequestBuilder *)requestBuilder completion:(void (^)(TLmessages_Dialogs *))completion failure:(void (^)(void))failure
{
    ...
    return [[TGTelegramNetworking instance] performRpc:getDialogs completionBlock:^(TLmessages_Dialogs *dialogs, __unused int64_t responseTime, MTRpcError *error)
    {
        if (error == nil)
        {
            IOS6Trace(@"IOS6TRACE dialogs archive success folder=%d dialogs=%d", folderId, (int)dialogs.dialogs.count);
            IOS6Trace(@"IOS6FULL RPC dialogs.archiveSuccess folder=%d dialogs=%d", folderId, (int)dialogs.dialogs.count);
            if (completion != nil)
                completion(dialogs);
        }
        else
        {
            IOS6Trace(@"IOS6TRACE dialogs archive failed folder=%d code=%d text=%@", folderId, error.errorCode, error.errorDescription);
            IOS6Trace(@"IOS6FULL RPC dialogs.archiveFailed folder=%d code=%d text=%@", folderId, error.errorCode, error.errorDescription);
            if (failure != nil)
                failure();
        }
    } progressBlock:nil requiresCompletion:true requestClass:TGRequestClassGeneric];
...
- (TLInputPeer *)createInputPeerForConversation:(int64_t)conversationId accessHash:(int64_t)accessHash
{
    if (conversationId == 0)
    {
        IOS6Trace(@"IOS6TRACE inputPeer peer=%lld type=empty hash=%lld", conversationId, accessHash);
        return [[TLInputPeer$inputPeerEmpty alloc] init];
    }
    else if (TGIOS6PeerIdIsChannel(conversationId, accessHash))
    {
        ...
        {
            TGLog(@"IOS6AUTH inputPeer accessHash repair peer=%lld old=%lld db=%lld", conversationId, accessHash, conversation.accessHash);
            accessHash = conversation.accessHash;
        }

        int64_t storedChannelId = TGIOS6StoredApiPeerId(conversationId, @"ios6ApiChannelId");
        TLInputPeer$inputPeerChannel *channelPeer = [[TLInputPeer$inputPeerChannel alloc] init];
        channelPeer.channel_id = storedChannelId > 0 ? storedChannelId : TGIOS6FallbackChannelIdFromPeerId(conversationId, accessHash);
        channelPeer.access_hash = accessHash;
        IOS6Trace(@"IOS6TRACE inputPeer peer=%lld type=channel modernRaw=%d id=%lld hash=%lld", conversationId, TGIOS6PeerIdIsModernRawChannel(conversationId, accessHash) ? 1 : 0, channelPeer.channel_id, accessHash);
        return channelPeer;
    }
    else if (conversationId < 0)
    {
        int64_t storedChatId = TGIOS6StoredApiPeerId(conversationId, @"ios6ApiChatId");
        if (storedChatId > 0)
        {
            TLInputPeer$inputPeerChat *chatPeer = [[TLInputPeer$inputPeerChat alloc] init];
            chatPeer.chat_id = storedChatId;
            IOS6Trace(@"IOS6TRACE inputPeer peer=%lld type=group64 chatId=%lld hash=%lld", conversationId, chatPeer.chat_id, accessHash);
            IOS6Trace(@"IOS6FULL inputPeer peer=%lld type=group64 chatId=%lld hash=%lld", conversationId, chatPeer.chat_id, accessHash);
            return chatPeer;
        }

        TGUser *modernUser = [TGDatabaseInstance() loadUser:(int)conversationId];
        if (modernUser != nil && modernUser.phoneNumberHash != 0)
        {
            TLInputPeer$inputPeerUser *foreignPeer = [[TLInputPeer$inputPeerUser alloc] init];
            foreignPeer.user_id = TGIOS6ModernUserIdFromStoredUser(modernUser, (int32_t)conversationId);
            foreignPeer.access_hash = modernUser.phoneNumberHash;
            IOS6Trace(@"IOS6TRACE inputPeer peer=%lld type=user-negative modern=%lld hash=%lld username=%@", conversationId, foreignPeer.user_id, modernUser.phoneNumberHash, modernUser.userName);
            return foreignPeer;
        }
        
        TLInputPeer$inputPeerChat *chatPeer = [[TLInputPeer$inputPeerChat alloc] init];
        chatPeer.chat_id = (int64_t)TGGroupIdFromPeerId(conversationId);
        IOS6Trace(@"IOS6TRACE inputPeer peer=%lld type=group chatId=%lld hash=%lld", conversationId, chatPeer.chat_id, accessHash);
        IOS6Trace(@"IOS6FULL inputPeer peer=%lld type=group chatId=%lld hash=%lld", conversationId, chatPeer.chat_id, accessHash);
        return chatPeer;
    }
    else if (conversationId == _clientUserId)
    {
        TLInputPeer$inputPeerSelf *selfPeer = [[TLInputPeer$inputPeerSelf alloc] init];
        IOS6Trace(@"IOS6TRACE inputPeer peer=%lld type=self hash=%lld", conversationId, accessHash);
        return selfPeer;
    }
    else
    {
        if (accessHash != 0) {
            ...
            IOS6Trace(@"IOS6TRACE inputPeer peer=%lld type=user modern=%lld hash=%lld source=argument", conversationId, foreignPeer.user_id, accessHash);
            return foreignPeer;
        } else {
            ...
            if (user != nil)
            {
                ...
                IOS6Trace(@"IOS6TRACE inputPeer peer=%lld type=user modern=%lld hash=%lld source=database", conversationId, foreignPeer.user_id, user.phoneNumberHash);
                return foreignPeer;
            }
        }

        IOS6Trace(@"IOS6TRACE inputPeer peer=%lld type=user result=empty missingAccessHash=1", conversationId);
        return [[TLInputPeer$inputPeerEmpty alloc] init];
    }
}
...
- (TLInputUser *)createInputUserForUid:(int)uid
{
    ...
    else
    {
        ...
        if (user != nil)
        {
            ...
            foreignUser.user_id = TGIOS6ModernUserIdFromStoredUser(user, (int32_t)uid);
            foreignUser.access_hash = user.phoneNumberHash;
            TGLog(@"IOS6AUTH inputUser bridge legacy=%d modern=%lld hash=%lld", uid, foreignUser.user_id, user.phoneNumberHash);
            return foreignUser;
        }
...
{
    NSData *invalidPeerMarker = [TGDatabaseInstance() conversationCustomPropertySync:conversationId name:murMurHash32(@"ios6InvalidPeer")];
    if (invalidPeerMarker.length != 0 && TGIOS6PeerIdIsChannel(conversationId, accessHash))
    {
        IOS6Trace(@"IOS6FULL RPC history.skipInvalidPeer peer=%lld maxMid=%d offset=%d limit=%d", conversationId, maxMid, offset, limit);
        if ([actor respondsToSelector:@selector(conversationHistoryRequestInvalidPeer:)])
            [actor conversationHistoryRequestInvalidPeer:@"IOS6_INVALID_PEER_CACHED"];
        else
            [actor conversationHistoryRequestFailed];
        return nil;
    }
    else if (invalidPeerMarker.length != 0)
    {
        IOS6Trace(@"IOS6FULL RPC history.ignoreInvalidPeerForBasicChat peer=%lld maxMid=%d offset=%d limit=%d", conversationId, maxMid, offset, limit);
    }
    
    IOS6Trace(@"IOS6TRACE history request peer=%lld hash=%lld maxMid=%d offset=%d limit=%d", conversationId, accessHash, maxMid, offset, limit);
    IOS6Trace(@"IOS6FULL RPC history.request peer=%lld hash=%lld maxMid=%d offset=%d limit=%d", conversationId, accessHash, maxMid, offset, limit);
    ...
    getHistory.min_id = 1;
    getHistory.max_id = maxMid > 0 ? maxMid : INT32_MAX;
    if (maxMid >= 0)
        getHistory.offset_id = maxMid;
    getHistory.add_offset = offset;
    getHistory.limit = limit;
    IOS6Trace(@"IOS6FULL RPC history.params peer=%lld min=%d max=%d offsetId=%d add=%d limit=%d", conversationId, getHistory.min_id, getHistory.max_id, getHistory.offset_id, getHistory.add_offset, getHistory.limit);
    ...
    return [[TGTelegramNetworking instance] performRpc:getHistory completionBlock:^(TLmessages_Messages *messages, __unused int64_t responseTime, MTRpcError *error)
    {
        if (error == nil)
        {
            ...
            int32_t minId = INT_MAX;
            int32_t maxId = 0;
            for (TLMessage *message in messages.messages)
            {
                if (message.n_id > 0)
                {
                    if (ids.length != 0)
                        [ids appendString:@","];
                    [ids appendFormat:@"%d", message.n_id];
                    minId = MIN(minId, message.n_id);
                    maxId = MAX(maxId, message.n_id);
                }
            }
            IOS6Trace(@"IOS6TRACE history success peer=%lld messages=%d chats=%d users=%d response=%@", conversationId, (int)messages.messages.count, (int)messages.chats.count, (int)messages.users.count, NSStringFromClass([messages class]));
            IOS6Trace(@"IOS6FULL RPC history.ids peer=%lld count=%d min=%d max=%d ids=%@", conversationId, (int)messages.messages.count, minId == INT_MAX ? 0 : minId, maxId, ids);
            IOS6Trace(@"IOS6FULL RPC history.success peer=%lld messages=%d chats=%d users=%d response=%@", conversationId, (int)messages.messages.count, (int)messages.chats.count, (int)messages.users.count, NSStringFromClass([messages class]));
            [actor conversationHistoryRequestSuccess:messages];
        }
        else
        {
            IOS6Trace(@"IOS6TRACE history failed peer=%lld code=%d text=%@", conversationId, error.errorCode, error.errorDescription);
            IOS6Trace(@"IOS6FULL RPC history.failed peer=%lld code=%d text=%@", conversationId, error.errorCode, error.errorDescription);
            NSString *errorDescription = [error.errorDescription uppercaseString];
            if (error.errorCode == 400 && ([errorDescription rangeOfString:@"PEER_ID_INVALID"].location != NSNotFound || [errorDescription rangeOfString:@"CHAT_ID_INVALID"].location != NSNotFound || [errorDescription rangeOfString:@"CHANNEL_INVALID"].location != NSNotFound))
            {
                if (TGIOS6PeerIdIsChannel(conversationId, accessHash))
                {
                    uint8_t one = 1;
                    [TGDatabaseInstance() setConversationCustomProperty:conversationId name:murMurHash32(@"ios6InvalidPeer") value:[NSData dataWithBytes:&one length:1]];
                    IOS6Trace(@"IOS6FULL RPC history.markInvalidPeer peer=%lld code=%d text=%@", conversationId, error.errorCode, error.errorDescription);
                }
                else
                {
                    IOS6Trace(@"IOS6FULL RPC history.basicChatInvalidNoCache peer=%lld code=%d text=%@", conversationId, error.errorCode, error.errorDescription);
                }
                
                if ([actor respondsToSelector:@selector(conversationHistoryRequestInvalidPeer:)])
                    [actor conversationHistoryRequestInvalidPeer:error.errorDescription];
                else
                    [actor conversationHistoryRequestFailed];
            }
            else
...
{
    IOS6Trace(@"IOS6TRACE mediaHistory request peer=%lld hash=%lld maxMid=%d maxDate=%d limit=%d", conversationId, accessHash, maxMid, maxDate, limit);
    ...
    return [[TGTelegramNetworking instance] performRpc:search completionBlock:^(TLmessages_Messages *messages, __unused int64_t responseTime, MTRpcError *error)
    {
        if (error == nil)
        {
            IOS6Trace(@"IOS6TRACE mediaHistory success peer=%lld messages=%d chats=%d users=%d response=%@", conversationId, (int)messages.messages.count, (int)messages.chats.count, (int)messages.users.count, NSStringFromClass([messages class]));
            [actor mediaHistoryRequestSuccess:messages];
        }
        else
        {
            IOS6Trace(@"IOS6TRACE mediaHistory failed peer=%lld code=%d text=%@", conversationId, error.errorCode, error.errorDescription);
...
- (NSObject *)doChangeConversationTitle:(int64_t)conversationId accessHash:(int64_t)accessHash title:(NSString *)title actor:(TGConversationChangeTitleRequestActor *)requestActor
{
    if (TGIOS6PeerIdIsChannel(conversationId, accessHash)) {
        ...
        TGIOS6FillInputChannel(inputChannel, conversationId, accessHash);
...
{
    NSData *invalidPeerMarker = [TGDatabaseInstance() conversationCustomPropertySync:conversationId name:murMurHash32(@"ios6InvalidPeer")];
    TGConversation *conversationForInvalidCheck = [TGDatabaseInstance() loadConversationWithId:conversationId];
    int64_t invalidCheckAccessHash = conversationForInvalidCheck != nil ? conversationForInvalidCheck.accessHash : 0;
    if (invalidPeerMarker.length != 0 && TGIOS6PeerIdIsChannel(conversationId, invalidCheckAccessHash))
    {
        IOS6Trace(@"IOS6FULL fullChat.skipInvalidPeer peer=%lld", conversationId);
        [actor chatFullRequestFailed];
        return nil;
    }
    else if (invalidPeerMarker.length != 0)
    {
        IOS6Trace(@"IOS6FULL fullChat.ignoreInvalidPeerForBasicChat peer=%lld", conversationId);
    }
    
    int64_t apiChatId = TGIOS6StoredApiPeerId(conversationId, @"ios6ApiChatId");
    if (conversationId < 0 && apiChatId == 0)
    {
        TGUser *modernUser = [TGDatabaseInstance() loadUser:(int)conversationId];
        if (modernUser != nil && modernUser.phoneNumberHash != 0)
        {
            IOS6Trace(@"IOS6TRACE skip getFullChat modern negative user peer=%lld modern=%lld hash=%lld username=%@", conversationId, TGIOS6ModernUserIdFromStoredUser(modernUser, (int32_t)conversationId), modernUser.phoneNumberHash, modernUser.userName);
            [actor chatFullRequestFailed];
            return nil;
        }
    }
    ...
    getFullChat.chat_id = apiChatId > 0 ? apiChatId : TGIOS6ChatIdFromPeerId(conversationId);
...
- (NSObject *)doChangePeerNotificationSettings:(int64_t)peerId accessHash:(int64_t)accessHash muteUntil:(NSNumber *)muteUntil soundId:(NSNumber *)soundId previewText:(NSNumber *)previewText messagesMuted:(NSNumber *)messagesMuted actor:(TGSynchronizeServiceActionsActor *)actor
{
    ...
    else
    {
        ...
        TGLog(@"IOS6MUTE rpc peer=%lld inputPeer=%@ accessHash=%lld muteUntil=%@", peerId, NSStringFromClass([inputPeer.peer class]), accessHash, muteUntil);
    }
    ...
    if (muteUntil != nil) {
        ...
        peerNotifySettings.mute_until = muteUntil.intValue;
    }
    ...
    return [[TGTelegramNetworking instance] performRpc:updatePeerNotifySettings completionBlock:^(TLPeerNotifySettings *result, __unused int64_t responseTime, MTRpcError *error)
    {
        if (error == nil)
        {
            TGLog(@"IOS6MUTE rpc success peer=%lld muteUntil=%@", peerId, muteUntil);
            [actor changePeerNotificationSettingsSuccess:result];
        }
        else
        {
            TGLog(@"IOS6MUTE rpc failed peer=%lld code=%d text=%@ accessHash=%lld", peerId, error.errorCode, error.errorDescription, accessHash);
...
{
    id rpc = nil;
    if (TGIOS6PeerIdIsChannel(peerId, accessHash)) {
        if (TGIOS6PeerIdIsModernRawChannel(peerId, accessHash)) {
            IOS6Trace(@"IOS6TRACE skip actor channels.getMessages modern peer=%lld channelId=%lld hash=%lld count=%d", peerId, TGIOS6ChannelIdFromPeerId(peerId, accessHash), accessHash, (int)mids.count);
            TLmessages_Messages$messages_messages *emptyMessages = [[TLmessages_Messages$messages_messages alloc] init];
            emptyMessages.messages = @[];
            emptyMessages.chats = @[];
            emptyMessages.users = @[];
            [actor messagesRequestSuccess:emptyMessages];
            return nil;
        }
        
        TLRPCchannels_getMessages$channels_getMessages *getMessages = [[TLRPCchannels_getMessages$channels_getMessages alloc] init];
        TLInputChannel$inputChannel *inputChannel = [[TLInputChannel$inputChannel alloc] init];
        TGIOS6FillInputChannel(inputChannel, peerId, accessHash);
        getMessages.channel = inputChannel;
        getMessages.n_id = TGIOS6ModernInputMessageIdsFromMessageIds(mids);
        rpc = getMessages;
    } else {
