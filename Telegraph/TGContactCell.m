    ...
    if (_avatarUrl.length != 0)
    {
        ...
        if (![_avatarUrl isEqualToString:_avatarView.currentUrl])
        {
            if (animateState)
            {
                ...
                [_avatarView loadImage:_avatarUrl filter:(TGIsPad() ? @"avatar45" : @"avatar40") placeholder:(currentImage != nil ? currentImage : placeholder) forceFade:true];
            }
            else
                [_avatarView loadImage:_avatarUrl filter:(TGIsPad() ? @"avatar45" : @"avatar40") placeholder:placeholder];
        }
    }
    else
    {
