...
- (void)startCallWithPeerId:(int64_t)__unused peerId uuid:(NSUUID *)__unused uuid
{
}

- (void)updateCallWithUUID:(NSUUID *)__unused uuid connectingAtDate:(NSDate *)__unused date
{
}

- (void)updateCallWithUUID:(NSUUID *)__unused uuid connectedAtDate:(NSDate *)__unused date
{
}

- (void)endCallWithUUID:(NSUUID *)__unused uuid reason:(TGCallDiscardReason)__unused reason completion:(void (^)(void))completion
{
    if (completion != nil)
        completion();
}

- (void)reportIncomingCallWithPeerId:(int64_t)__unused peerId session:(TGCallSession *)__unused session uuid:(NSUUID *)__unused uuid completion:(void (^)(bool))completion
{
    if (completion != nil)
        completion(false);
}

- (void)addCallSession:(TGCallSession *)__unused session uuid:(NSUUID *)__unused uuid
{
}
