...
{
    id value = message.contentProperties[@"ios6ReactionSummary"];
    ...
    // Compatibility with messages created in memory by the short-lived broken build.
    if ([value isKindOfClass:[NSString class]])
        return value;
    return nil;
}

static NSString *TGIOS6ChosenReactionFromMessage(TGMessage *message)
{
    id value = message.contentProperties[@"ios6ReactionSummary"];
    if ([value isKindOfClass:[TGMessageReactionSummaryContentProperty class]])
...
static UIImage *TGIOS6ReactionButtonImage(NSString *emoji, NSInteger count, bool selected)
{
    if (emoji.length == 0)
        return nil;
...
- (void)_ios6RebuildReactionButtons:(TGModernViewStorage *)viewStorage
{
    ...
    NSString *chosenKey = TGIOS6ReactionKey(_ios6ChosenReaction);
    __weak TGContentBubbleViewModel *weakSelf = self;
    for (NSDictionary *item in TGIOS6ReactionItems(_reactionSummary))
    {
        NSString *emoji = [item[@"emoji"] copy];
        NSInteger count = [item[@"count"] integerValue];
        bool selected = chosenKey.length != 0 && [chosenKey isEqualToString:TGIOS6ReactionKey(emoji)];
        TGModernButtonViewModel *button = [[TGModernButtonViewModel alloc] init];
        button.image = TGIOS6ReactionButtonImage(emoji, count, selected);
        ...
        button.pressed = ^
        {
            ...
            if (strongSelf == nil)
                return;
            bool remove = [TGIOS6ReactionKey(strongSelf->_ios6ChosenReaction) isEqualToString:TGIOS6ReactionKey(emoji)];
    ...
    UIView *container = [_contentModel boundView].superview;
    if (container != nil)
    {
        for (TGModernButtonViewModel *button in _ios6ReactionButtonModels)
        {
    ...
    if (self != nil)
    {
        ...
        if (authorPeer != nil)
        {
            ...
            if ([authorPeer isKindOfClass:[TGUser class]]) {
                ...
                if (title.length == 0 && ((TGUser *)authorPeer).userName.length != 0)
                    title = ((TGUser *)authorPeer).userName;
                if (title.length == 0)
                    title = [NSString stringWithFormat:@"%d", ((TGUser *)authorPeer).uid];
            } else if ([authorPeer isKindOfClass:[TGConversation class]]) {
                title = ((TGConversation *)authorPeer).chatTitle;
                if (title.length == 0)
                    title = [NSString stringWithFormat:@"%lld", ((TGConversation *)authorPeer).conversationId];
            }
        ...
        NSString *reactionSummary = TGIOS6ReactionSummaryFromMessage(message);
        _reactionSummary = [reactionSummary copy];
        _ios6ChosenReaction = [TGIOS6ChosenReactionFromMessage(message) copy];
        ...
        {            
            if (!_inhibitChecks)
            {
                ...
                _checkFirstModel = [[TGModernImageViewModel alloc] initWithImage:deliveredIcon];
                _checkSecondModel = [[TGModernImageViewModel alloc] initWithImage:readIcon];
            }
...
- (void)updateMessage:(TGMessage *)message viewStorage:(TGModernViewStorage *)viewStorage sizeUpdated:(bool *)sizeUpdated
{
    ...
    NSString *newReactionSummary = TGIOS6ReactionSummaryFromMessage(message);
    NSString *newChosenReaction = TGIOS6ChosenReactionFromMessage(message);
    bool reactionSummaryChanged = !TGObjectCompare(_reactionSummary, newReactionSummary) || !TGObjectCompare(_ios6ChosenReaction, newChosenReaction);
    if (reactionSummaryChanged)
    {
        _reactionSummary = [newReactionSummary copy];
        _ios6ChosenReaction = [newChosenReaction copy];
        int daytimeVariant = 0;
        NSString *dateText = [TGDateUtils stringForShortTime:(int)message.date daytimeVariant:&daytimeVariant];
        if (!debugShowMessageIds)
        ...
        if (_reactionSummary.length == 0)
        {
            // Remove a label left by builds that rendered the reaction twice.
            if (_reactionLabelModel != nil)
            {
                [_contentModel removeSubmodel:_reactionLabelModel viewStorage:viewStorage];
                _reactionLabelModel = nil;
            }
        }
...
- (void)layoutForContainerSize:(CGSize)containerSize
{
    ...
    if (!_incoming) {
        if (_messageViews == nil) {
            ...
            infoWidth += MAX(0.0f, 20.0f - _messageViewsModel.frame.size.width);
            if (!isPost) {
                infoWidth += 20.0f;
            }
        }
    ...
    // A long reaction row must never make the text column narrower/taller.
    // The capsule is clipped to the already calculated bubble width below.
    CGFloat backgroundWidth = MAX(60.0f, MAX(MAX(headerSize.width, contentSize.width) + 25.0f, infoWidth + 20.0f));
    bool classicIOS6Style = true;
    ...
    {
        CGFloat maximumAvailableWidth = containerSize.width - layoutConstants->leftInset - layoutConstants->rightInset - avatarOffset - unsentOffset;
        backgroundWidth = MAX(backgroundWidth, MIN(maximumAvailableWidth, floorf(containerSize.width * 0.75f)));
    }
    // bubbleMine has a long, non-content tail on the right.  Widen the
    // background to the left and reserve that same width inside so the whole
    // content/date/check block moves together without changing its layout.
    backgroundWidth += classicOutgoingTailInset;
    ...
    if (classicIOS6Style)
    {
        // The content/date container consumes two pixels of the nominal
        // bottom padding.  Add seven pixels so the visible bottom inset is
        // ten pixels: twice the five-pixel top inset requested for Skeuo.
        backgroundFrame.size.height += 7.0f;
    }
    ...
    CGFloat contentLeftInset = _incomingAppearance ? 14.0f : 8.0f;
    CGFloat contentWidth = MAX(32.0f, backgroundFrame.size.width - contentLeftInset - classicOutgoingTailInset);
    if (_inhibitContentAnimation)
    {
        [UIView performWithoutAnimation:^
        {
             _contentModel.frame = CGRectMake(backgroundFrame.origin.x + contentLeftInset, topSpacing + 2.0f, contentWidth, MAX(headerSize.height + contentSize.height + 5, (_hasAvatar && webPageSize.height <= FLT_EPSILON) ? 30.0f : (webpageOnly ? 0.0 : 14.0f)) + webPageSize.height);
        }];
    }
    else
    {
    ...
    if (_ios6ReactionButtonModels.count != 0)
    {
        ...
        CGFloat x = _incomingAppearance ? backgroundFrame.origin.x + 8.0f : MAX(8.0f, CGRectGetMaxX(backgroundFrame) - 8.0f - totalWidth);
    ...
    self.frame = CGRectMake(0, 0, containerSize.width, backgroundFrame.size.height + topSpacing + bottomSpacing + reactionExtraHeight + replyButtonsHeight);
