...
{
    if (true)
        return [TGNavigationBarPallete palleteWithBackgroundColor:UIColorRGB(0x6688ad) separatorColor:UIColorRGB(0x294768) titleColor:[UIColor whiteColor] tintColor:[UIColor whiteColor]];
...
static NSString *const TGClassicIOS6NativeBubblesDefaultsKey = @"TGClassicIOS6NativeBubbles";
...
{
    {
        ...
        bool classicStyle = true;
        bool nativeClassicButtons = classicStyle && iosMajorVersion() <= 6;
        UIColor *navigationButtonColor = classicStyle ? [UIColor whiteColor] : TGPresentation.current.pallete.navigationButtonColor;
        UIColor *navigationTitleColor = classicStyle ? [UIColor whiteColor] : TGPresentation.current.pallete.navigationTitleColor;
        UIColor *navigationShadowColor = classicStyle ? UIColorRGBA(0x203b58, 0.9f) : [UIColor clearColor];
        
        UIBarButtonItem *item = [UIBarButtonItem appearanceWhenContainedIn:[TGNavigationBar class], nil];

        UIImage *backImage = nil;
        UIImage *backHighlightedImage = nil;
        UIImage *backLandscapeImage = nil;
        UIImage *backLandscapeHighlightedImage = nil;
        if (nativeClassicButtons)
        {
            // On iOS 6, nil restores UIKit's real glossy UIBarButtonItem
            // artwork. This is both more authentic and safer than emulating it.
        ...
        {
            [item setBackgroundImage:TGClassicIOS6NavigationButtonImage(false, false) forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
            [item setBackgroundImage:TGClassicIOS6NavigationButtonImage(false, true) forState:UIControlStateHighlighted barMetrics:UIBarMetricsDefault];
            backImage = TGClassicIOS6NavigationButtonImage(true, false);
            backHighlightedImage = TGClassicIOS6NavigationButtonImage(true, true);
            backLandscapeImage = backImage;
            backLandscapeHighlightedImage = backHighlightedImage;
        }
        else
        {
            ...
            backImage = TGTintedImage([UIImage imageNamed:@"NavigationBackButton.png"], navigationButtonColor);
            backHighlightedImage = TGTintedImage([UIImage imageNamed:@"NavigationBackButton_Highlighted.png"], navigationButtonColor);
            backLandscapeImage = TGTintedImage([UIImage imageNamed:@"NavigationBackButtonLandscape.png"], navigationButtonColor);
            backLandscapeHighlightedImage = TGTintedImage([UIImage imageNamed:@"NavigationBackButtonLandscape_Highlighted.png"], navigationButtonColor);
        }
        if (nativeClassicButtons)
        {
            ...
            [item setBackButtonBackgroundImage:nil forState:UIControlStateNormal barMetrics:UIBarMetricsLandscapePhone];
            [item setBackButtonBackgroundImage:nil forState:UIControlStateHighlighted barMetrics:UIBarMetricsLandscapePhone];
            [item setBackButtonTitlePositionAdjustment:UIOffsetZero forBarMetrics:UIBarMetricsDefault];
            [item setBackButtonTitlePositionAdjustment:UIOffsetZero forBarMetrics:UIBarMetricsLandscapePhone];
        }
        else
        {
            int backCapWidth = classicStyle ? 20 : (int)backImage.size.width;
            int backLandscapeCapWidth = classicStyle ? 20 : (int)backLandscapeImage.size.width;
            int backTopCapHeight = classicStyle ? 14 : 0;
            [item setBackButtonBackgroundImage:[backImage stretchableImageWithLeftCapWidth:backCapWidth topCapHeight:backTopCapHeight] forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
            [item setBackButtonBackgroundImage:[backHighlightedImage stretchableImageWithLeftCapWidth:backCapWidth topCapHeight:backTopCapHeight] forState:UIControlStateHighlighted barMetrics:UIBarMetricsDefault];
            [item setBackButtonBackgroundImage:[backLandscapeImage stretchableImageWithLeftCapWidth:backLandscapeCapWidth topCapHeight:backTopCapHeight] forState:UIControlStateNormal barMetrics:UIBarMetricsLandscapePhone];
            [item setBackButtonBackgroundImage:[backLandscapeHighlightedImage stretchableImageWithLeftCapWidth:backLandscapeCapWidth topCapHeight:backTopCapHeight] forState:UIControlStateHighlighted barMetrics:UIBarMetricsLandscapePhone];
        ...
        UIColor *buttonTitleColor = classicStyle ? navigationButtonColor : TGPresentation.current.pallete.accentColor;
        // Native iOS 6 bar buttons use a visibly smaller caption than the
        // navigation title.  Keeping both at 16 pt made short captions such
        // as "Chats" look oversized inside the glossy back-button shape.
        [item setTitleTextAttributes:@{UITextAttributeTextColor:buttonTitleColor, UITextAttributeTextShadowColor:navigationShadowColor, UITextAttributeTextShadowOffset:[NSValue valueWithUIOffset:UIOffsetMake(0.0f, -1.0f)], UITextAttributeFont: classicStyle ? TGBoldSystemFontOfSize(12.0f) : TGSystemFontOfSize(16.0f)} forState:UIControlStateNormal];
        [item setTitleTextAttributes:@{UITextAttributeTextColor:[buttonTitleColor colorWithAlphaComponent:0.55f], UITextAttributeTextShadowColor:navigationShadowColor, UITextAttributeTextShadowOffset:[NSValue valueWithUIOffset:UIOffsetMake(0.0f, -1.0f)], UITextAttributeFont:classicStyle ? TGBoldSystemFontOfSize(12.0f) : TGSystemFontOfSize(16.0f)} forState:UIControlStateHighlighted];
        
        [[TGNavigationBar appearance] setTitleTextAttributes:@{UITextAttributeTextColor:navigationTitleColor, UITextAttributeTextShadowColor:navigationShadowColor, UITextAttributeTextShadowOffset:[NSValue valueWithUIOffset:UIOffsetMake(0.0f, -1.0f)], UITextAttributeFont:TGBoldSystemFontOfSize(17.0f)}];
...
+ (bool)classicIOS6NativeBubbles
{
    return true;
}

+ (void)setClassicIOS6NativeBubbles:(bool)enabled
{
}
