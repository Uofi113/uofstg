...
#import "../submodules/LegacyComponents/WatchConnectivity/WatchConnectivity.h"
...
static Class TGSystemWCSessionClass(void)
{
    if (iosMajorVersion() < 9)
        return Nil;

    return NSClassFromString(@"WCSession");
}

static bool TGSystemWCSessionIsSupported(Class sessionClass)
{
    SEL selector = NSSelectorFromString(@"isSupported");
    if (sessionClass == Nil || ![sessionClass respondsToSelector:selector])
        return false;

    @try
    {
        BOOL (*implementation)(id, SEL) = (BOOL (*)(id, SEL))[sessionClass methodForSelector:selector];
        return implementation != NULL && implementation(sessionClass, selector);
    }
...
static id TGSystemWCSessionDefaultSession(void)
{
    Class sessionClass = TGSystemWCSessionClass();
    SEL selector = NSSelectorFromString(@"defaultSession");
    if (sessionClass == Nil || ![sessionClass respondsToSelector:selector])
        return nil;

    @try
    {
        id (*implementation)(id, SEL) = (id (*)(id, SEL))[sessionClass methodForSelector:selector];
        return implementation == NULL ? nil : implementation(sessionClass, selector);
    }
    @catch (__unused NSException *exception)
    {
        return nil;
...
{
    [[TGBridgeServer queue] dispatch:^{
        int64_t peerId = [userInfo[@"peerId"] longLongValue];
        int32_t messageId = [userInfo[@"messageId"] intValue];
...
- (WCSession *)session
{
    return (WCSession *)TGSystemWCSessionDefaultSession();
}
