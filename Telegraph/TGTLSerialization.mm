...
+ (id)parseResponse:(NSData *)data request:(TLMetaRpc *)request
{
    ...
    {
        TGLogTLParseError(error);
        NSLog(@"IOS6TLRESPONSE top=0x%08x request=%@ implied=0x%08x description=%@", topSignature, NSStringFromClass([request class]), request.impliedResponseSignature, error.localizedDescription ?: @"");
    }
