...
@interface TGCollectionMenuController () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, TGCollectionMenuViewDelegate, TGCollectionMenuLayoutDataSource, TGCollectionMenuLayoutDelegateFlowLayout>
{
    ...
    __weak TGNavigationController *_weakNavController;
}
...
- (void)setPresentation:(TGPresentation *)presentation
{
    ...
    if ([self isViewLoaded])
        self.view.backgroundColor = TGCollectionMenuClassicIOS6BackgroundColor();
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    _weakNavController = (TGNavigationController *)self.navigationController;
    
    if (_editingMode)
    {
        if (iosMajorVersion() >= 7)
            _weakNavController.interactivePopGestureRecognizer.enabled = false;
    }
}
